 ...
 * http://stackoverflow.com/questions/4599786/android
 * -linkify-both-web-and-mentions-all-in-the-same-textview
 */

package com.tweetlanes.android.util;
...
public class TwitterLinkify {
    ...
    /**
     * Bit field indicating that twitter @mentions should be matched in methods
     * that take an options mask
     */
    ...
    public static final int ALL = WEB_URLS | EMAIL_ADDRESSES | PHONE_NUMBERS
            | AT_MENTIONS | HASH_TAGS | MAP_ADDRESSES;
    public static final int TWITTER_ALL = WEB_URLS | AT_MENTIONS | HASH_TAGS;
    ...
    public static final boolean addLinks(Spannable text, int mask) {
        ...
        if ((mask & WEB_URLS) != 0) {
            gatherLinks(links, text, Regex.WEB_URL_PATTERN, new String[] {
                    "http://", "https://" }, sUrlMatchFilter, null);
        }
        ...
        if ((mask & AT_MENTIONS) != 0) {
            gatherLinks(links, text, Pattern.compile("@([A-Za-z0-9_-]+)"),
                    new String[] { "com.tweetlanes.android.profile://" }, null,
                    null);
        }

        if ((mask & HASH_TAGS) != 0) {
            gatherLinks(links, text, Pattern.compile("#([A-Za-z0-9_-]+)"),
                    new String[] { "com.tweetlanes.android.search://" }, null,
                    null);
        }
...
// TODO: Default members are BAD! Replace this class with a more OOP style class
