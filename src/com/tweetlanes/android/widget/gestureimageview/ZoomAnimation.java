    ...
    public boolean update(GestureImageView view, long time) {
        if (mFirstFrame) {
            ...
            if (mScaleDiff > 0) {
                ...
                vector.mLength = length * mZoom;

                // Now deduce the new endpoint
                vector.calculateEndPoint();

                mXDiff = vector.mEnd.x - mStartX;
                mYDiff = vector.mEnd.y - mStartY;
            } else {
                // Zoom out to center
        ...
        if (ratio < 1) {
            ...
            if (ratio > 0) {
                ...
                if (mZoomAnimationListener != null) {
                    mZoomAnimationListener.onZoom(newScale, newX, newY);
                }
            }
            ...
            if (mZoomAnimationListener != null) {
                mZoomAnimationListener.onZoom(newScale, newX, newY);
                mZoomAnimationListener.onComplete();
            }
