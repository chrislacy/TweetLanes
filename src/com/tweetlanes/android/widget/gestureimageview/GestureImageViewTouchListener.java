...
public class GestureImageViewTouchListener implements OnTouchListener {
    ...
    private void startFling() {
        ...
        mImageView.animationStart(mFlingAnimation);
    }
    ...
    private void startZoom(MotionEvent e) {
        ...
        if (mImageView.isLandscape()) {
            if (mImageView.getDeviceOrientation() == Configuration.ORIENTATION_PORTRAIT) {
                int scaledHeight = mImageView.getScaledHeight();

                if (scaledHeight < mCanvasHeight) {
                    zoomTo = mFitScaleVertical / mCurrentScale;
                    mZoomAnimation.setTouchX(e.getX());
                    mZoomAnimation.setTouchY(mImageView.getCenterY());
                } else {
                    zoomTo = mFitScaleHorizontal / mCurrentScale;
                    mZoomAnimation.setTouchX(mImageView.getCenterX());
                    mZoomAnimation.setTouchY(mImageView.getCenterY());
                }
            } else {
                int scaledWidth = mImageView.getScaledWidth();
        ...
        mImageView.animationStart(mZoomAnimation);
    }

    private void stopAnimations() {
        mImageView.animationStop();
    }
                ...
                if (event.getAction() == MotionEvent.ACTION_UP) {
                    ...
                    if (event.getPointerCount() > 1) {
                        ...
                        if (mInitialDistance > 0) {
                            ...
                            float distance = mPinchVector.mLength;
                            ...
                            if (mInitialDistance != distance) {
                                ...
                                if (newScale <= mMaxScale) {
                                    mScaleVector.mLength *= newScale;

                                    mScaleVector.calculateEndPoint();

                                    mScaleVector.mLength /= newScale;

                                    float newX = mScaleVector.mEnd.x;
                                    float newY = mScaleVector.mEnd.y;
                            ...
                            mScaleVector.mLength /= mLastScale;
                        }
                    } else {
                        if (!mTouched) {
                            mTouched = true;
                            mLast.x = event.getX();
                            mLast.y = event.getY();
                            mNext.x = mImageView.getImageX();
                            mNext.y = mImageView.getImageY();
                        } else if (!mMultiTouch) {
    ...
    protected void handleUp() {
        ...
        mImageView.setScale(mCurrentScale);
        mImageView.setPosition(mNext.x, mNext.y);
