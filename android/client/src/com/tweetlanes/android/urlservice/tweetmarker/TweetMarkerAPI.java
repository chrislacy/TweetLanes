...
package com.tweetlanes.android.urlservice.tweetmarker;
...
import com.tweetlanes.android.urlservice.ApiService;
...
public class TweetMarkerAPI extends ApiService {
	...
	public static HttpResponse getRequest(String url, String debugName) {
		...
		request.addHeader("X-Verify-Credentials-Authorization", TwitterManager.get().generateTwitterVerifyCredentialsAuthorizationHeader());
	 ...
	 * 
	 */
	public static void getLastRead(AccountDescriptor account, final ConnectionStatus.Callbacks connectionStatus, final APICallback callback) {
		...
		AsyncTaskEx<AccountDescriptor, Void, TaskOutput> worker = new AsyncTaskEx<AccountDescriptor, Void, TaskOutput>() {
			...
			protected TaskOutput doInBackground(AccountDescriptor... inputArray) {
				...
				if (connectionStatus.isOnline() == false) {
					return new TaskOutput(new TwitterFetchResult(false, connectionStatus.getErrorMessageNoConnection()), null);
				}
				...
				return new TaskOutput(new TwitterFetchResult(errorDescription == null ? true : false, errorDescription), jsonAsString);
			}
