...
public class AccountDescriptor {
	...
	private void initCommon(ArrayList<String> displayedLanes) {
		...
		if (Constant.ENABLE_PROFILE_IMAGES) {
			...
			URLFetch.fetchBitmap(TwitterManager.getProfileImageUrl(mScreenName, TwitterManager.ProfileImageSize.BIGGER), callback);
		}
	}
	...
	private void configureLaneDefinitions(ArrayList<String> displayedLanes) {
		...
		mLaneDefinitions.add(new LaneDescriptor(Constant.LaneType.USER_PROFILE, App.getContext().getString(R.string.lane_user_profile), 
				new TwitterContentHandleBase(TwitterConstant.ContentType.USER)));
		mLaneDefinitions.add(new LaneDescriptor(Constant.LaneType.USER_PROFILE_TIMELINE, App.getContext().getString(R.string.lane_user_tweets),
		...
		if (Constant.SOCIAL_NET_TYPE == SocialNetConstant.Type.Twitter) {
			mLaneDefinitions.add(new LaneDescriptor(Constant.LaneType.USER_PROFILE_TIMELINE, App.getContext().getString(R.string.lane_user_retweets_of_me),
					new TwitterContentHandleBase(TwitterConstant.ContentType.STATUSES, TwitterConstant.StatusesType.RETWEETS_OF_ME)));
		}
		
		mLaneDefinitions.add(new LaneDescriptor(Constant.LaneType.USER_HOME_TIMELINE, App.getContext().getString(R.string.lane_user_home), 
				new TwitterContentHandleBase(TwitterConstant.ContentType.STATUSES, TwitterConstant.StatusesType.USER_HOME_TIMELINE)));

		mLaneDefinitions.add(new LaneDescriptor(Constant.LaneType.USER_MENTIONS, App.getContext().getString(R.string.lane_user_mentions),
				new TwitterContentHandleBase(TwitterConstant.ContentType.STATUSES, TwitterConstant.StatusesType.USER_MENTIONS)));

		if (Constant.SOCIAL_NET_TYPE == SocialNetConstant.Type.Appdotnet) {
			mLaneDefinitions.add(new LaneDescriptor(Constant.LaneType.GLOBAL_FEED, App.getContext().getString(R.string.lane_user_global_feed),
					new TwitterContentHandleBase(TwitterConstant.ContentType.STATUSES, TwitterConstant.StatusesType.GLOBAL_FEED)));
		}
		
		if (Constant.SOCIAL_NET_TYPE == SocialNetConstant.Type.Twitter) {
			mLaneDefinitions.add(new LaneDescriptor(Constant.LaneType.DIRECT_MESSAGES, App.getContext().getString(R.string.lane_direct_messages),
					new TwitterContentHandleBase(TwitterConstant.ContentType.DIRECT_MESSAGES, TwitterConstant.DirectMessagesType.ALL_MESSAGES)));
			
			// Add lists
			for (List list : mLists) {
				if (list.mId != null) {
					mLaneDefinitions.add(new LaneDescriptor(Constant.LaneType.USER_LIST_TIMELINE, list.mName, String.valueOf(list.mId),
							new TwitterContentHandleBase(TwitterConstant.ContentType.STATUSES, TwitterConstant.StatusesType.USER_LIST_TIMELINE)));
				}
			}
			
			// Add the final batch
			mLaneDefinitions.add(new LaneDescriptor(Constant.LaneType.FRIENDS, App.getContext().getString(R.string.lane_friends),
					new TwitterContentHandleBase(TwitterConstant.ContentType.USERS, TwitterConstant.UsersType.FRIENDS)));
			mLaneDefinitions.add(new LaneDescriptor(Constant.LaneType.FOLLOWERS, App.getContext().getString(R.string.lane_followers),
					new TwitterContentHandleBase(TwitterConstant.ContentType.USERS, TwitterConstant.UsersType.FOLLOWERS)));
			mLaneDefinitions.add(new LaneDescriptor(Constant.LaneType.USER_FAVORITES, App.getContext().getString(R.string.lane_user_favorites),
	...
	public String 	getOAuthToken()		{ return mOAuthToken; }
	public String 	getOAuthSecret()	{ return mOAuthSecret; }
	
	public Bitmap	getProfileImage()	{ return Constant.ENABLE_PROFILE_IMAGES ? mProfileImage : null; }
	
	/*
	 * 
	 */
	public int	getInitialLaneIndex() { 
		if (mInitialLaneIndex != null && mInitialLaneIndex.intValue() < getDisplayedLaneDefinitionsSize()) {
	...
	private Bitmap mProfileImage;		// Of size TwitterManager.ProfileImageSize.BIGGER
	...
	/*
	 * Stripped version of the List class. Possibly should use TwitterList, but I thought I thought it best to save the string space of that much larger structure
	 */
	private class List {
