...
public class App extends Application {
	...
	public	static int	getAppVersionNumber()	{ return mAppVersionNumber; }
	public	static String getAppVersionName()	{ return mAppVersionName; }
	public 	static boolean getActionLauncherInstalled() { return mActionLauncherInstalled; }
	...
	private OAuthLoginState				mLoginState;
	public 	OAuthLoginState				getOAuthLoginState()						{ return mLoginState; }
	public 	void						setOAuthLoginState(OAuthLoginState state) 	{
		Intent intent = new Intent("" + SystemEvent.OAuthLoginStateChange);
		LocalBroadcastManager.getInstance(this).sendBroadcast(intent);
		mLoginState = state; 
	}
	*/
	
	public 	AccountDescriptor			getCurrentAccount()				{ return mCurrentAccountIndex != null ? mAccounts.get(mCurrentAccountIndex) : null; }
	public  String						getCurrentAccountScreenName()	{ AccountDescriptor account = getCurrentAccount(); return account != null ? account.getScreenName() : null; }
	public  Integer						getLastAccountKey()				{ return mLastAccountIndex; }
	public 	int							getAccountCount()				{ return mAccounts.size(); }
	
	public	ArrayList<AccountDescriptor>	getAccounts()		{ 	return mAccounts;		}
	...
	public 	void setCurrentAccount(Long id)		{ 
		...
		if (mCurrentAccountIndex == null) {
			...
			if (account != null) {
				TwitterManager.get().setOAuthTokenWithSecret(account.getOAuthToken(), account.getOAuthSecret(), true);
				
				final Editor edit = mPreferences.edit();
				edit.putLong(SHARED_PREFERENCES_KEY_CURRENT_ACCOUNT_ID, account.getId());
				edit.commit();
			} else {
				// TODO: Handle me
			}
	...
	public	int							getProfileLaneDefaultIndex()		{ return mProfileLaneDefaultIndex; }
	public 	ArrayList<LaneDescriptor>	getProfileLaneDefinitions()			{ return mProfileLaneDefinitions; }
	public 	LaneDescriptor				getProfileLaneDescriptor(int index)	{ return mProfileLaneDefinitions.get(index); }

	public	int							getSearchLaneDefaultIndex()			{ return mSearchLaneDefaultIndex; }
	public 	ArrayList<LaneDescriptor>	getSearchLaneDefinitions()			{ return mSearchLaneDefinitions; }
	public 	LaneDescriptor				getSearchLaneDescriptor(int index)	{ return mSearchLaneDefinitions.get(index); }
	
	public 	int							getTweetSpotlightLaneDefaultIndex() { return mTweetSpotlightLaneDefaultIndex; }
	public 	ArrayList<LaneDescriptor>	getTweetSpotlightLaneDefinitions() 	{ return mTweetSpotlightLaneDefinitions; }
	public 	LaneDescriptor				getTweetSpotlightLaneDescriptor(int index) { return mTweetSpotlightLaneDefinitions.get(index); }
	
	public 	SharedPreferences			getPreferences()				{ return mPreferences; }
	...
	private 		String	getAccountDescriptorKey(Long id)		{ return "account_descriptor_v2" + id.toString(); }
	private			String	getTweetDraftKey()						{ return "draft_" + getCurrentAccountScreenName(); }
	private			String	getTweetDraftTimeKey()						{ return "drafttime_" + getCurrentAccountScreenName(); }
	...
	public String getTweetDraftAsString() {
		...
		if (saveTime > 0) {
			...
			if (secondsDiff < Constant.RESTORE_SAVED_DRAFT_SECONDS) {
				String draftAsString = mPreferences.getString(getTweetDraftKey(), null);
	...
	public boolean getTutorialCompleted() {
		boolean tutorialCompleted = mPreferences.getBoolean(SHARED_PREFERENCES_KEY_TUTORIAL_COMPLETED, false);
	...
	public void saveUpdatedAccountDescriptor(AccountDescriptor account) {
		final Editor edit = mPreferences.edit();
		edit.putString(getAccountDescriptorKey(account.getId()), account.toString());
		edit.commit();
	}
	...
	public void onPostSignIn(TwitterUser user, String oAuthToken, String oAuthSecret) {
		...
		if (user != null) {
			...
			try {
				...
				AccountDescriptor account = new AccountDescriptor(user, oAuthToken, oAuthSecret);
	...
	public void onCreate() {
		...
		try {
	        ...
	        for (ApplicationInfo app : apps) {
	        	if (app.packageName != null && app.packageName.equalsIgnoreCase("com.chrislacy.actionlauncher.pro")) {
		...
		mProfileLaneDefinitions.add(new LaneDescriptor(Constant.LaneType.PROFILE_PROFILE, getContext().getString(R.string.lane_profile_profile), 
				new TwitterContentHandleBase(TwitterConstant.ContentType.USER)));
		mProfileLaneDefinitions.add(new LaneDescriptor(Constant.LaneType.PROFILE_PROFILE_TIMELINE, getContext().getString(R.string.lane_profile_tweets),
				new TwitterContentHandleBase(TwitterConstant.ContentType.STATUSES, TwitterConstant.StatusesType.USER_TIMELINE)));
		mProfileLaneDefinitions.add(new LaneDescriptor(Constant.LaneType.PROFILE_MENTIONS, getContext().getString(R.string.lane_profile_mentions),
				new TwitterContentHandleBase(TwitterConstant.ContentType.STATUSES, TwitterConstant.StatusesType.SCREEN_NAME_SEARCH)));
		if (Constant.SOCIAL_NET_TYPE == SocialNetConstant.Type.Twitter) {
			mProfileLaneDefinitions.add(new LaneDescriptor(Constant.LaneType.PROFILE_FAVORITES, getContext().getString(R.string.lane_profile_favorites),
					new TwitterContentHandleBase(TwitterConstant.ContentType.STATUSES, TwitterConstant.StatusesType.USER_FAVORITES)));
		}
		mProfileLaneDefaultIndex = 0;
		
		mSearchLaneDefinitions = new ArrayList<LaneDescriptor>();
		
		mSearchLaneDefinitions.add(new LaneDescriptor(Constant.LaneType.SEARCH_TERM, getContext().getString(R.string.lane_search_tweets),
				new TwitterContentHandleBase(TwitterConstant.ContentType.STATUSES, TwitterConstant.StatusesType.STATUS_SEARCH)));
		if (Constant.SOCIAL_NET_TYPE == SocialNetConstant.Type.Twitter) {
			mSearchLaneDefinitions.add(new LaneDescriptor(Constant.LaneType.SEARCH_PERSON, getContext().getString(R.string.lane_search_people),
					new TwitterContentHandleBase(TwitterConstant.ContentType.USERS, TwitterConstant.UsersType.PEOPLE_SEARCH)));
			mSearchLaneDefaultIndex = 0;
		}
		
		mTweetSpotlightLaneDefinitions = new ArrayList<LaneDescriptor>();
		mTweetSpotlightLaneDefinitions.add(new LaneDescriptor(Constant.LaneType.STATUS_SPOTLIGHT, getContext().getString(R.string.lane_tweet_status), 
				new TwitterContentHandleBase(TwitterConstant.ContentType.STATUS, TwitterConstant.StatusType.GET_STATUS)));
		mTweetSpotlightLaneDefinitions.add(new LaneDescriptor(Constant.LaneType.STATUS_CONVERSATION, getContext().getString(R.string.lane_tweet_conversation),
				new TwitterContentHandleBase(TwitterConstant.ContentType.STATUS, TwitterConstant.StatusesType.FULL_CONVERSATION)));
		mTweetSpotlightLaneDefinitions.add(new LaneDescriptor(Constant.LaneType.STATUS_RETWEETED_BY, getContext().getString(R.string.lane_tweet_retweeted_by),
				new TwitterContentHandleBase(TwitterConstant.ContentType.USERS, TwitterConstant.UsersType.RETWEETED_BY)));
		...
		if (currentAccountDescriptor != null) {
			TwitterManager.initModule(Constant.SOCIAL_NET_TYPE, Constant.CONSUMER_KEY, Constant.CONSUMER_SECRET, 
					currentAccountDescriptor.getOAuthToken(), currentAccountDescriptor.getOAuthSecret(), mConnectionStatusCallbacks);
		} else {
			TwitterManager.initModule(Constant.SOCIAL_NET_TYPE, Constant.CONSUMER_KEY, Constant.CONSUMER_SECRET, null, null, mConnectionStatusCallbacks);
		}
	...
	public ConnectionStatus.Callbacks getConnectionStatusCallbacks()		{ return mConnectionStatusCallbacks; }
	 ...
	 * 
	 */
	public void triggerFollowPromoAccounts(TwitterFetchUsers.FinishedCallback callback) {
		ArrayList<Long> userIds = new ArrayList<Long>();
		userIds.add(Constant.USER_ID_CHRISMLACY);
		userIds.add(Constant.USER_ID_TWEETLANES);
    	TwitterManager.get().updateFriendshipUserIds(getCurrentAccount().getId(), userIds, true, callback);
	}
	...
	public void restartApp(Activity currentActivity) {
		...
		intent.addFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP|Intent.FLAG_ACTIVITY_NO_ANIMATION|Intent.FLAG_ACTIVITY_CLEAR_TASK);
	...
	public LazyImageLoader getPreviewImageLoader() {
		if (mPreviewImageLoader == null) {
			...
			mPreviewImageLoader = new LazyImageLoader(this, Constant.DIR_NAME_CACHED_THUMBNAILS,
					R.drawable.white, preview_image_width, preview_image_height, 30);
		}
	...
	public LazyImageLoader getProfileImageLoader() {
		if (mProfileImageLoader == null) {
			final int profile_image_size = getResources().getDimensionPixelSize(R.dimen.avatar_width_height_large);
			mProfileImageLoader = new LazyImageLoader(this, Constant.DIR_NAME_PROFILE_IMAGES,
					R.drawable.ic_contact_picture, profile_image_size, profile_image_size, 60);
		}
