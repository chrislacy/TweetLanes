...
public class App extends Application {
	...
	return mCurrentAccountIndex != null ? mAccounts.get(mCurrentAccountIndex) : null;
    }
	...
	if (mCurrentAccountIndex == null) {
		...
		if (account.getSocialNetType() == currentSocialNetType) {
		    TwitterManager.get().setOAuthTokenWithSecret(account.getOAuthToken(),
		    ...
		    TwitterManager
			    .initModule(
				    ...
				    account.getSocialNetType() == SocialNetConstant.Type.Appdotnet ? Constant.APPDOTNET_CONSUMER_SECRET
					    : Constant.TWITTER_CONSUMER_SECRET, account
					    .getOAuthToken(), account.getOAuthSecret(),
	...
	String accountIndices = mPreferences
		.getString(SHARED_PREFERENCES_KEY_ACCOUNT_INDICES, null);
		...
		TwitterManager
			.initModule(
				...
				oSocialNetType == SocialNetConstant.Type.Twitter ? Constant.TWITTER_CONSUMER_SECRET
					: Constant.APPDOTNET_CONSUMER_SECRET, oAuthToken,
				oAuthSecret, mConnectionStatusCallbacks);
	    }
	}
    }
	    ...
	    TwitterManager
		    .initModule(
			    ...
			    socialNetType == SocialNetConstant.Type.Twitter ? Constant.TWITTER_CONSUMER_SECRET
				    : Constant.TWITTER_CONSUMER_SECRET, currentAccountDescriptor
				    .getOAuthToken(), currentAccountDescriptor.getOAuthSecret(),
			    mConnectionStatusCallbacks);
	} else {
	    TwitterManager.initModule(SocialNetConstant.Type.Twitter,
		    Constant.TWITTER_CONSUMER_KEY, Constant.TWITTER_CONSUMER_SECRET, null, null,
		    mConnectionStatusCallbacks);
	}
	...
	mProfileLaneDefinitions.add(new LaneDescriptor(Constant.LaneType.PROFILE_PROFILE,
		getContext().getString(R.string.lane_profile_profile),
		new TwitterContentHandleBase(TwitterConstant.ContentType.USER)));
	mProfileLaneDefinitions.add(new LaneDescriptor(Constant.LaneType.PROFILE_PROFILE_TIMELINE,
		getContext().getString(R.string.lane_profile_tweets), new TwitterContentHandleBase(
			TwitterConstant.ContentType.STATUSES,
			TwitterConstant.StatusesType.USER_TIMELINE)));
	mProfileLaneDefinitions.add(new LaneDescriptor(Constant.LaneType.PROFILE_MENTIONS,
		getContext().getString(R.string.lane_profile_mentions),
		new TwitterContentHandleBase(TwitterConstant.ContentType.STATUSES,
			TwitterConstant.StatusesType.SCREEN_NAME_SEARCH)));
	mProfileLaneDefinitions.add(new LaneDescriptor(Constant.LaneType.PROFILE_FAVORITES,
		getContext().getString(R.string.lane_profile_favorites),
		new TwitterContentHandleBase(TwitterConstant.ContentType.STATUSES,
	...
	mSearchLaneDefinitions.add(new LaneDescriptor(Constant.LaneType.SEARCH_TERM, getContext()
		.getString(R.string.lane_search_tweets), new TwitterContentHandleBase(
		TwitterConstant.ContentType.STATUSES, TwitterConstant.StatusesType.STATUS_SEARCH)));
	if (socialNetType == SocialNetConstant.Type.Twitter) {
	    mSearchLaneDefinitions.add(new LaneDescriptor(Constant.LaneType.SEARCH_PERSON,
		    getContext().getString(R.string.lane_search_people),
	...
	mTweetSpotlightLaneDefinitions
		.add(new LaneDescriptor(Constant.LaneType.STATUS_SPOTLIGHT, getContext().getString(
			R.string.lane_tweet_status), new TwitterContentHandleBase(
			TwitterConstant.ContentType.STATUS, TwitterConstant.StatusType.GET_STATUS)));
	mTweetSpotlightLaneDefinitions.add(new LaneDescriptor(
		Constant.LaneType.STATUS_CONVERSATION, getContext().getString(
			R.string.lane_tweet_conversation), new TwitterContentHandleBase(
			TwitterConstant.ContentType.STATUS,
			TwitterConstant.StatusesType.FULL_CONVERSATION)));
	mTweetSpotlightLaneDefinitions
		.add(new LaneDescriptor(Constant.LaneType.STATUS_RETWEETED_BY, getContext()
			.getString(R.string.lane_tweet_retweeted_by), new TwitterContentHandleBase(
			TwitterConstant.ContentType.USERS, TwitterConstant.UsersType.RETWEETED_BY)));
	...
	TwitterManager.get().updateFriendshipUserIds(getCurrentAccount().getId(), userIds, true,
		callback);
    }
	...
	if (mPreviewImageLoader == null) {
	    final int preview_image_width = getResources().getDimensionPixelSize(
		    R.dimen.image_preview_width);
	    final int preview_image_height = getResources().getDimensionPixelSize(
		    R.dimen.image_preview_height);
	...
	if (mProfileImageLoader == null) {
	    final int profile_image_size = getResources().getDimensionPixelSize(
		    R.dimen.avatar_width_height_large);
