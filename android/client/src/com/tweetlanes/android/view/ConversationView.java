...
public class ConversationView extends LinearLayout {
	...
	public void configure(TwitterStatus twitterStatus, LayoutInflater inflater, Callbacks callbacks) {
		...
		TwitterContentHandleBase base = new TwitterContentHandleBase(TwitterConstant.ContentType.STATUSES, TwitterConstant.StatusesType.PREVIOUS_CONVERSATION);
		mContentHandle = new TwitterContentHandle(base, twitterStatus.mUserScreenName, Long.valueOf(mTwitterStatus.mId).toString());
		...
		if (cachedStatuses != null && cachedStatuses.getStatusCount() > 0) {
			...
			TwitterManager.get().triggerFetchStatuses(mContentHandle, null, new TwitterFetchStatusesFinishedCallback() {
	...
	private void updateViewVisibility(boolean loadHasFinished) {
		...
		if (loadHasFinished == false && (mConversationStatuses == null || mConversationStatuses.getStatusCount() == 0)) {
