...
import com.tweetlanes.android.view.DirectMessageItemView.DirectMessageItemViewCallbacks;
...
public class DirectMessageFeedFragment extends BaseLaneFragment {
	 ...
	 * 
	 */
	public static DirectMessageFeedFragment newInstance(int laneIndex, final TwitterContentHandleBase handleBase, final String screenName, final String laneIdentifier, final Long otherUserId) {
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
		...
		mDirectMessagesHandle = new TwitterDirectMessagesHandle(getBaseLaneActivity().getApp().getCurrentAccount().getId(), getOtherUserId());
		...
		//mMultipleConversationSelectionCallback = new MultipleConversationSelectionCallback();
		mConversationListView = (PullToRefreshListView) resultView.findViewById(R.id.pull_to_refresh_listview);
		mConversationListView.getRefreshableView().setOnItemClickListener(mTweetFeedOnItemClickListener);
		mConversationListView.getRefreshableView().setChoiceMode(ListView.CHOICE_MODE_NONE);
		...
		if (mDirectMessageConversation == null || mDirectMessageConversation.size() == 0) {
	...
	private TwitterContentHandleBase getContentHandleBase() { return (TwitterContentHandleBase) getArguments().getSerializable("handleBase"); }
	private String getScreenName() 			{ return getArguments().getString("screenName"); }
	private String getLaneIdentifier()		{ return getArguments().getString("laneIdentifier"); }
	private Long getOtherUserId()			{ return getArguments().containsKey("otherUserId") ? getArguments().getLong("otherUserId") : null; }
	...
	private void setDirectMessages(TwitterDirectMessages directMessages) {
		...
		if (directMessages == null || directMessages.getConversationCount() == 0) {
			...
			mDirectMessageConversation = directMessages.getList(mDirectMessagesHandle);
			//_mDirectMessages = new TwitterDirectMessages(directMessages, getOtherUserId());
			
			Long newestDirectMessageId = directMessages.getNewestDirectMessageId();
			if (newestDirectMessageId != null) {
				mNewestDirectMessageId = newestDirectMessageId;
			}

			Long oldestDirectMessageId = directMessages.getOldestDirectMessageId();
	...
	private void updateViewVisibility(boolean loadHasFinished) {
		...
		if (loadHasFinished == false && (mDirectMessageConversation == null || mDirectMessageConversation.size() == 0)) {
	...
	private OnScrollListener mOnScrollListener = new OnScrollListener(){
		...
		public void onScroll(AbsListView view, int firstVisibleItem, int visibleItemCount, int totalItemCount) {
			mConversationListView.onScroll(view, firstVisibleItem, visibleItemCount, totalItemCount);
		}
	...
	private OnLastItemVisibleListener mOnLastItemVisibleListener = new OnLastItemVisibleListener() {
		...
		public void onLastItemVisible() {
			...
			mRefreshCallback = new TwitterFetchDirectMessagesFinishedCallback() {
				@Override
				public void finished(TwitterFetchResult result, TwitterDirectMessages feed) {
					
					onRefreshComplete(feed);
					
					if (mRefreshingDirectMessageId.longValue() == mOldestDirectMessageId.longValue()) {
			...
			if (mOldestDirectMessageId != null) {
				if (mRefreshingDirectMessageId == null) {
					...
					if (count > 2) {
						...
						TwitterManager.get().getDirectMessages(mContentHandle, TwitterPaging.createGetOlder(mOldestDirectMessageId), mRefreshCallback);
					}
				}
			}
		}
		...
		public void onRefresh() {
			...
			if (mNewestDirectMessageId != null) {
				if (mRefreshingNewestDirectMessageId == null) {
					...
					TwitterDirectMessages directMessages = TwitterManager.get().getDirectMessages(mContentHandle, TwitterPaging.createGetNewer(mNewestDirectMessageId), mRefreshCallback);
					if (directMessages == null) {
						getBaseLaneActivity().finishCurrentActionMode();
					}
				}
			} else {
				TwitterManager.get().getDirectMessages(mContentHandle, null, mRefreshCallback);
				getBaseLaneActivity().finishCurrentActionMode();
			}
		}
	};
	...
	private void onDirectMessageItemViewClick(View view, int position) {
		...
		if (getOtherUserId() == null) {
			...
			TwitterDirectMessage directMessage = directMessageItemView.getDirectMessage();
			
			DirectMessageActivity.createAndStartActivity(getActivity(), mContentHandle,
					directMessage.getOtherUserId(), directMessage.getOtherUserScreenName());
		}
		...
		//	getBaseLaneActivity().setComposeDefault(new ComposeTweetDefault(getApp().getCurrentAccountScreenName(), getSelectedUsers()));
		//} else {
    ...
    private class DirectMessageConversationListAdapter extends BaseAdapter {
        ...
        public int getCount() {
        	...
        	if (mDirectMessageConversation != null) {
        		if (getOtherUserId() == null) {
        			count = mDirectMessageConversation.size() + 1;		// +1 for the trailing LoadMore view
        		} else {
        ...
        public View getView(int position, View convertView, ViewGroup parent) {
        	...
        	int directMessageCount = mDirectMessageConversation != null ? mDirectMessageConversation.size() : 0;
        ...
        View getDirectMessageFeedItemView(int position, View convertView) {
        	...
        	TwitterDirectMessage directMessage = mDirectMessageConversation.get(position);
        	
        	MessageType messageType = MessageType.RECEIVED;
        	
        	Long otherUserId = getOtherUserId();
        	if (otherUserId != null && directMessage.getMessageType() == MessageType.SENT) {
        		messageType = MessageType.SENT;
        		//convertView = mInflater.inflate(R.layout.direct_message_feed_item_sent, null);
        	}
        	convertView = mInflater.inflate(R.layout.direct_message_feed_item_received, null);
        	
        	DirectMessageItemView directMessageItemView = (DirectMessageItemView)convertView.findViewById(R.id.directMessageItem);
	    	...
	    	directMessageItemView.configure(getScreenName(), directMessage, position+1, messageType, otherUserId == null ? false : true, callbacks);
        ...
        View getLoadMoreView(View convertView) {
        	...
        	if (mDirectMessageConversation == null || mDirectMessageConversation.size() == 0) {
        		...
        		mode = mMoreDirectMessagesAvailable == true ? LoadMoreView.Mode.LOADING : LoadMoreView.Mode.NO_MORE;
        	}
	...
	public void triggerInitialDownload() {
		...
		mRefreshCallback  = new TwitterFetchDirectMessagesFinishedCallback() {
			...
			@Override
			public void finished(TwitterFetchResult fetchResult, TwitterDirectMessages directMessages) {
		...
		TwitterDirectMessages cachedDirectMessages = TwitterManager.get().getDirectMessages(mContentHandle, null, mRefreshCallback);
		if (cachedDirectMessages != null) {
			if (mRefreshCallback != null) {
				mRefreshCallback.finished(new TwitterFetchResult(true, null), cachedDirectMessages);
			}
		} else {
