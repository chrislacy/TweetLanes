...
public abstract class BaseLaneFragment extends Fragment {
	 ...
	 * 
	 */
	public void configureBaseLaneFragment(int laneIndex, String identifier, ConfigureBundleListener configureBundleListener) { 
	 ...
	 * 
	 */
	public 	InitialDownloadState getInitialDownloadState()	{ return mInitialDownloadState; }
		...
		if (laneContent != null) {
			...
			if (tweetFeedLoaded != null) {
				if (AppSettings.get().showTabletMargin() == false) {
					...
					laneContent.setLayoutParams(new RelativeLayout.LayoutParams(parentParams.width, parentParams.height));
				} else {
					/*
