...
public class BootActivity extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		//LocalBroadcastManager.getInstance(this).registerReceiver(mOAuthLoginStateChangeReceiver, new IntentFilter("" + SystemEvent.OAuthLoginStateChange));
	}
	 ...
	 */
	/*
	private BroadcastReceiver mOAuthLoginStateChangeReceiver = new BroadcastReceiver() {
		  @Override
	...
	*/
	...
	void showTermsDialog() {
		if (mTermsDialog == null) {
			...
			mTermsDialog = alertDialogBuilder.create();
			mTermsDialog.show();
		}
	}*/
	...
	void jumpToNext() {
		...
		if (accountCount == 0) {
			...
			Intent intent = new Intent(getApplicationContext(), Constant.SOCIAL_NET_TYPE == SocialNetConstant.Type.Twitter ? TwitterAuthActivity.class : AppDotNetAuthActivity.class);
			overridePendingTransition(0, 0);
			intent.setFlags(Intent.FLAG_ACTIVITY_NO_ANIMATION);
			startActivity(intent);
			...
			if (getApp().getOAuthLoginState() == OAuthLoginState.NONE) {
				...
				mLastStartedClass = PrepareRequestTokenActivity.class;
