...
public class UserFeedFragment extends BaseLaneFragment {
	 ...
	 * 
	 */
	public static UserFeedFragment newInstance(int laneIndex, final TwitterContentHandleBase handleBase, final String screenName, final String laneIdentifier) {
			...
			if (mOldestUserId != null) {
				if (mRefreshingUserTweetId == null) {
					...
					if (count > 2) {
						...
						TwitterManager.get().getUsers(mContentHandle, TwitterPaging.createGetOlder(mOldestUserId), mUserDataRefreshCallback);
					}
				}
			}
		}
		...
		public void onRefresh() {
			...
			if (mNewestUserId != null) {
				if (mRefreshingNewestUserId == null) {
					...
					TwitterUsers users = TwitterManager.get().getUsers(mContentHandle, TwitterPaging.createGetNewer(mNewestUserId), mUserDataRefreshCallback);
					if (users == null) {
	...
	private void onUserFeedItemViewClick(int position, boolean checked) {
		...
		if (mSelectedItems.size() > 0) {
			getBaseLaneActivity().setComposeTweetDefault(new ComposeTweetDefault(getApp().getCurrentAccountScreenName(), getSelectedUsers()));
		} else {
			getBaseLaneActivity().setComposeDefault();
		}
	}
	...
	public void triggerInitialDownload() {
		...
		TwitterUsers cachedUsers = null;
		if (mContentHandle.getUsersType() == UsersType.RETWEETED_BY) {
			cachedUsers = TwitterManager.get().getUsers(mContentHandle, TwitterPaging.createGetMostRecent(20), mUserDataRefreshCallback);
		} else {
