...
public class ComposeTweetFragment extends ComposeBaseFragment {
	...
	/*
	 * (non-Javadoc)
	 * @see com.tweetlanes.android.view.ComposeBaseFragment#onCreateView(android.view.LayoutInflater, android.view.ViewGroup, android.os.Bundle)
	 */
	@Override
	public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
		View resultView = super.onCreateView(inflater, container, savedInstanceState);
		
		mAttachImagePreview = (ImageView)resultView.findViewById(R.id.statusImage);
	 ...
	 * 
	 */
	FinishedCallback mOnSetStatusCallback = TwitterManager.get().getFetchStatusInstance().new FinishedCallback() {
		...
		public void finished(TwitterFetchResult result, TwitterStatus status) {
			...
			if (mSendingNotification != null) {
				NotificationHelper.get().cancel(getActivity(), mSendingNotification);
				mSendingNotification = null;
			}
			...
			*/
			...
			if (result.isSuccessful()) {
				...
				/*
				if (Constant.SHOW_NOTIFICATION_AFTER_TWEET_POSTED) {
					Intent i = new Intent(getActivity().getApplicationContext(), TweetSpotlightActivity.class);
					i.putExtra(TweetSpotlightActivity.STATUS_ID_KEY, Long.toString(status.getId()));
			        ...
			        builder.setContentTitle(getString(R.string.tweet_posted_success));
				...
				*/
					if (getActivity() != null && getActivity().getApplicationContext() != null) {
						Toast.makeText(getActivity().getApplicationContext(), getString(R.string.tweet_posted_success), Constant.DEFAULT_TOAST_DISPLAY_TIME).show();
					}
				//}
	...
	String getDefaultQuoteStatus(TwitterStatus statusToQuote) {
		if (statusToQuote.mStatus.length() > 0) {
			...
			switch (AppSettings.get().getCurrentQuoteType()) {
				...
				case RT:
					quote = "RT @" + statusToQuote.getAuthorScreenName() + ": " + statusToQuote.mStatus;
					break;
					
				case Via:
					quote = statusToQuote.mStatus + " (via @" + statusToQuote.getAuthorScreenName() + ")";
					break;
			
				case Standard:
				default:
					quote = "\"@" + statusToQuote.getAuthorScreenName() + ": " + statusToQuote.mStatus + "\"";
					break;
			}
			return quote;
		}
		...
		if (currentStatus != null && currentStatus.equals("") == false) {
			if (mStatusValidator.getTweetLength(currentStatus) > 0) {
				if (getComposeTweetDefault() != null) {
					...
					composeDraft = new ComposeTweetDefault(getApp().getCurrentAccountScreenName(), currentStatus);
				}
			}
		}
	...
	/*
	 * (non-Javadoc)
	 * @see com.tweetlanes.android.view.ComposeBaseFragment#clearCompose(boolean)
	 */
	@Override
	...
	protected void onSendClick(String status) {
		if (status != null) {
			...
			if (mStatusValidator.isValidTweet(status) == false) {
				...
				// Too long:
				//status = "Hi there. Hi there. Hi there. Hi there. Hi there. Hi there. Hi there. Hi there. Hi there. Hi there. Hi there. Hi there. Hi there. Hi there. Hi there. Hi there. Hi there. ";
				TwitterStatusUpdate statusUpdate = new TwitterStatusUpdate(status, getInReplyToId());
				...
				TwitterManager.get().setStatus(statusUpdate, mOnSetStatusCallback);
				
				/*
		        NotificationHelper.Builder builder = NotificationHelper.get().new Builder(getActivity(), true);
		        builder.setContentTitle(getString(R.string.posting_tweet_ongoing));
		...
		if (composeTweetDefault != null) {
			...
			if (lastStatus != null) {
				...
				if (composeTweetDefault.getInReplyToStatusId() != null) {
					hint = getString(R.string.compose_tweet_reply_to) + " " + getStatusHintSnippet(lastStatus, 20);
				} else {
					String[] words = lastStatus.split(" ");
					if (words.length == 1 && words[0].length() > 0 && words[0].charAt(0) == '@') {
						hint = getString(R.string.compose_tweet_to_user_hint) + " " + getStatusHintSnippet(words[0], 18);
					} else {
						hint = getString(R.string.compose_tweet_finish) + " \"" + getStatusHintSnippet(lastStatus, 16) + "\"";
					}
				}
			}
		}
	...
	void setMediaFilePath(String filePath) {
		...
		if (getComposeTweetDefault() == null) {
			setComposeTweetDefault(new ComposeTweetDefault(getApp().getCurrentAccountScreenName(), null, filePath));
		} else {
		...
		if (_mComposeDefault != null && _mComposeDefault.getMediaFilePath() != null) {
			...
			if(imgFile.exists()){
				try {
				    ...
				    mAttachImagePreview.setImageBitmap(Bitmap.createScaledBitmap(bitmap, 200, 200, false));
	...
	public void retweetSelected(TwitterStatus status) {
	    ...
	    alertDialog.setButton3(getString(R.string.quote), new DialogInterface.OnClickListener() {  
	    	public void onClick(DialogInterface dialog, int which) {  
	    ...
	    alertDialog.setButton(getString(R.string.retweet), new DialogInterface.OnClickListener() {  
	    	public void onClick(DialogInterface dialog, int which) {
    			if (mRetweetStatus != null) {
    				TwitterManager.get().setRetweet(mRetweetStatus.mId, null);
    				mRetweetStatus = null;
    			}
	    	}
	    });
	...
	/*
	 * (non-Javadoc)
	 * @see com.tweetlanes.android.view.ComposeBaseFragment#getLayoutResourceId()
	 */
	@Override
