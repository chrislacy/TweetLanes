...
public class ComposeTweetFragment extends ComposeBaseFragment {
	...
	if (Util.isValidString(currentStatus)) {
	    composeTweetDefault = new ComposeTweetDefault(getApp().getCurrentAccountScreenName(),
		    currentStatus, getInReplyToId(), getMediaFilePath());
	}
	...
	if (status != null) {
		...
		ComposeTweetDefault currentStatus = new ComposeTweetDefault(getApp()
			.getCurrentAccountScreenName(), status, getInReplyToId(),
			getMediaFilePath());
	...
	if (getComposeTweetDefault() == null) {
	    setComposeTweetDefault(new ComposeTweetDefault(getApp().getCurrentAccountScreenName(),
		    null, filePath));
	} else {
	...
	if (_mComposeDefault != null && _mComposeDefault.getMediaFilePath() != null) {
		...
		try {
		    ...
		    mAttachImagePreview.setImageBitmap(Bitmap.createScaledBitmap(bitmap, 200, 200,
			    false));
