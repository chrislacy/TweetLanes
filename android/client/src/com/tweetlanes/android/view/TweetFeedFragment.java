...
public final class TweetFeedFragment extends BaseLaneFragment {
	...
	public static TweetFeedFragment newInstance(int laneIndex, final TwitterContentHandleBase handleBase, final String screenName, final String laneIdentifier) {
		...
		fragment.configureBaseLaneFragment(laneIndex, fragment.mContentHandle.getTypeAsString(), new ConfigureBundleListener() {
	...
	private TwitterStatuses _mStatusFeed;			// Don't touch me directly. Use the accessors
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
		...
		mContentHandle = TwitterManager.get().getContentHandle(getContentHandleBase(), getScreenName(), getLaneIdentifier());
		...
		LocalBroadcastManager.getInstance(getActivity()).registerReceiver(mVolumeUpKeyDownReceiver, new IntentFilter("" + SystemEvent.VOLUME_UP_KEY_DOWN));
		LocalBroadcastManager.getInstance(getActivity()).registerReceiver(mVolumeDownKeyDownReceiver, new IntentFilter("" + SystemEvent.VOLUME_DOWN_KEY_DOWN));
	...
	void fetchNewestTweets(final long sinceStatusId, Long maxStatusId) {
		if (mTweetDataRefreshCallback == null) {
			...
			TwitterManager.get().triggerFetchStatuses(mContentHandle, paging, mTweetDataRefreshCallback, getAsyncTaskPriorityOffset());
	...
	public String getDataToCache() {
		...
		if (mTweetFeedListAdapter != null && mTweetFeedListView != null) {
			...
			//int endIndex = Math.min(visibleIndex + 5, Math.min(getStatusFeed().getStatusCount(), mTweetFeedListAdapter.getCount()));
			...
			for (int i = startIndex; i < endIndex; i++) {
				...
				if (i == visibleIndex) {
					...
					//Log.d("StatusCache", "Set visible: " + status.getStatus());
				}
	...
	public void triggerInitialDownload() {
		...
		TwitterManager.get().triggerFetchStatuses(mContentHandle, null, mTweetDataRefreshCallback, getAsyncTaskPriorityOffset());
	...
	private class ScrollTracker {
		...
		void update(int firstVisibleItem, int totalItemCount, Integer yOffset) {
			...
			//Log.d("Statuses", "Direction: " + mLastScrollDirection.toString());
		}
		
		ScrollDirection getLastScrollDirection() 	{ return mLastScrollDirection; }
		int getFirstVisibleYOffset()				{ return mFirstVisibleYOffset; }
	};
			...
			if (mOldestTweetId != null) {
				if (mRefreshingOldestTweetId == null) {
					if (mTweetDataLoadMoreCallback == null) {
						...
						TwitterManager.get().triggerFetchStatuses(mContentHandle, TwitterPaging.createGetOlder(mOldestTweetId), 
		...
		if (mTwitterStatusIdWhenRefreshed != null && firstVisibleItem > 1) {
			if (mScrollTracker.getLastScrollDirection() == ScrollDirection.TO_OLDER) {
				...
				if (mHidingListHeading == false) {
					...
					mListHeadingTextView.setText((firstVisibleItem-1) + " " + getString(firstVisibleItem == 2 ? R.string.new_tweet : R.string.new_tweets));
				}
			}
		} else {
			setListHeadingVisiblilty(View.GONE);
		...
		if (getStatusFeed() != null && mTweetFeedListView != null && mTweetFeedListView.getRefreshableView() != null) {
			...
			if (visiblePosition < getStatusFeed().getStatusCount()) {
				...
				if (visibleStatus != null) {
					//Log.d("Statuses", "Visible Status: " + Util.getPrettyDate(visibleStatus.getCreatedAt()) + ", " + visibleStatus.getStatus());
				}
			}
		}
	...
	private void onRefreshFinished(TwitterFetchResult result, TwitterStatuses feed) {
		...
		if (newCount > 0) {
			if (visibleStatus != null) {
				...
				if (statusIndex != null) {
					mTweetFeedListView.getRefreshableView().setSelectionFromTop(statusIndex.intValue(), mScrollTracker.getFirstVisibleYOffset());
				}
			}
		}
		mTweetDataRefreshCallback = null;
	...
	public boolean onOptionsItemSelected(MenuItem item) {
		...
		case R.id.action_replies_visibility:
			getBaseLaneActivity().mStatusesFilter.setShowReplies(!getBaseLaneActivity().mStatusesFilter.getShowReplies());
	 		item.setTitle(getString(getBaseLaneActivity().mStatusesFilter.getShowReplies() ? R.string.action_hide_replies : R.string.action_show_replies));
	 		mTweetFeedListAdapter.notifyDataSetChanged();
		 	return false;
		 		
		case R.id.action_retweets_visibility:
			getBaseLaneActivity().mStatusesFilter.setShowRetweets(!getBaseLaneActivity().mStatusesFilter.getShowRetweets());
			item.setTitle(getString(getBaseLaneActivity().mStatusesFilter.getShowRetweets() ? R.string.action_hide_retweets : R.string.action_show_retweets));
			mTweetFeedListAdapter.notifyDataSetChanged();
			return false;
		}
	...
	/*
	 * (non-Javadoc)
	 * @see com.tweetlanes.android.view.BaseLaneFragment#configureOptionsMenu(android.view.Menu)
	 */
	@Override
					...
					if (subMenu != null) {
						...
						for (int j = 0; j < subMenuSize; j++) {
							...
							switch (subMenuItem.getItemId()) {
								case R.id.action_replies_visibility:
									subMenuItem.setTitle(getString(getBaseLaneActivity().mStatusesFilter.getShowReplies() ? R.string.action_hide_replies : R.string.action_show_replies));
									break;
									
								case R.id.action_retweets_visibility:
									subMenuItem.setTitle(getString(getBaseLaneActivity().mStatusesFilter.getShowRetweets() ? R.string.action_hide_retweets : R.string.action_show_retweets));
									break;
	...
	private TwitterContentHandleBase getContentHandleBase() { return (TwitterContentHandleBase) getArguments().getSerializable(KEY_HANDLE_BASE); }
	private String getScreenName() 			{ return getArguments().getString(KEY_SCREEN_NAME); }
	private String getLaneIdentifier()		{ return getArguments().getString(KEY_LANE_IDENTIFIER); }
	...
	private void setStatusFeed(TwitterStatuses statuses, boolean addCachedStatuses) {
		...
		//int oldSize = _mStatusFeed != null ? _mStatusFeed.getStatusCount() : 0;
		...
		//Log.d("Statuses", "Old: " + oldSize + ", New: " + newSize + ", Cached: " + (_mCachedStatusFeed != null ? _mCachedStatusFeed.getStatusCount() : 0));
	...
	private void onTweetFeedItemLongPress(View view, int position) {
		...
		boolean isChecked = mTweetFeedListView.getRefreshableView().getCheckedItemPositions().get(position) == true ? true : false;
		...
		if (mSelectedItems.size() > 0 && getApp() != null) {
			mMultipleTweetSelectionCallback.setIsFavorited(getSelectedFavoriteState() == ItemSelectedState.ALL ? true : false);
			getBaseLaneActivity().setComposeTweetDefault(new ComposeTweetDefault(getApp().getCurrentAccountScreenName(), getSelectedStatuses()));
		} else {
    ...
    private class MultipleTweetSelectionCallback implements ListView.MultiChoiceModeListener {
        ...
        public boolean onActionItemClicked(ActionMode mode, MenuItem item) {
            ...
            switch (itemId) {
            	...
            	case R.id.action_reply:
            		getBaseLaneActivity().beginCompose();
            		break;
            		*/
            	...
            	{
        			TwitterModifyStatuses.FinishedCallback callback = TwitterManager.get().getSetStatusesInstance().new FinishedCallback() {
            	...
            	case R.id.action_block:
            	{
            		...
            		if (account != null) {
            			...
            			if (selected != null && selected.getStatusCount() > 0) {
            				...
            				TwitterFetchUsers.FinishedCallback callback = TwitterManager.get().getFetchUsersInstance().new FinishedCallback() {
								...
								public void finished(TwitterFetchResult result, TwitterUsers users) {
									...
									if (result.isSuccessful() && users != null && users.getUserCount() > 0) {
										...
										if (itemId == R.id.action_report_for_spam) {
											if (userCount == 1) {
												notice = "Reported @" + users.getUser(0).getScreenName() +  " for Spam.";
											} else {
        ...
        void configure(ActionMode mode) {
        	...
        	if (baseLaneActivity == null 
        		|| baseLaneActivity.isComposing()
        		|| mTweetFeedListView == null
        ...
        void setIsFavorited(boolean favorited) {
        	if (mFavoriteMenuItem != null) {
	        	...
	        	if (favorited) {
					mFavoriteMenuItem.setIcon(isDarkTheme ? R.drawable.ic_action_star_on_dark : R.drawable.ic_action_star_on_light);
					mFavoriteMenuItem.setTitle(R.string.action_unfavorite);
				} else {
					mFavoriteMenuItem.setIcon(isDarkTheme ? R.drawable.ic_action_star_off_dark : R.drawable.ic_action_star_off_light);
    ...
    private class TweetFeedListAdapter extends BaseAdapter {
        ...
        public int getCount() {
        	int count;
        	if (getStatusFeed() != null) {
        		count = getFilteredStatusCount() + 1;		// +1 for the trailing LoadMore view
        	} else {
        		count = 1;
        	}
        ...
        View getTweetFeedView(int position, View convertView) {
				...
				public void onUrlClicked(TwitterStatus status) {
					...
					if (selected != null && selected.getStatusCount() == 1) {
						if (selected.getStatus(0).mId == status.mId) {
							...
							// HACK ALERT: Finishing the actionMode in this callback will cause the link to be unable to be clicked. 
	    	...
	    	tweetFeedItemView.configure(item, position+1, callbacks, true, showRetweetCount, showConversationView(item), false, true);
