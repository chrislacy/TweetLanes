...
public class DirectMessageItemView extends LinearLayout {
	...
	public DirectMessageItemView(Context context, AttributeSet attrs, int defStyle) {
	 ...
	 * 
	 */
	public void configure(String userScreenName, TwitterDirectMessage directMessage, int position, 
			MessageType messageType, boolean fullConversation, DirectMessageItemViewCallbacks callbacks) {
		...
		if (mAuthorScreenNameTextView != null) {
			mAuthorScreenNameTextView.setText("@" + (messageType == MessageType.SENT ? userScreenName : directMessage.getOtherUserScreenName()));
		...
		if (mPrettyDateTextView != null) {
			mPrettyDateTextView.setText(Util.getPrettyDate(directMessage.getCreatedAt()));
		}
		...
		if (AppSettings.get().downloadFeedImages()) {
			String imageUrl = TwitterManager.getProfileImageUrl((messageType == MessageType.SENT ? userScreenName : directMessage.getOtherUserScreenName()), TwitterManager.ProfileImageSize.BIGGER);
	...
	public TwitterDirectMessage getDirectMessage() { return mDirectMessage; }
	...
	public void onProfileImageClick() {
		...
		if (mDirectMessage != null) {
			if (mFullConversation == true && mDirectMessage.getMessageType() == MessageType.SENT) {
				AccountDescriptor account = ((App)mCallbacks.getActivity().getApplication()).getCurrentAccount();
				if (account != null) {
					profileIntent.putExtra("userId", Long.valueOf(account.getId()));
					profileIntent.putExtra("userScreenName", account.getScreenName());
				}
			}
			else {
				profileIntent.putExtra("userId", Long.valueOf(mDirectMessage.getOtherUserId()));
				profileIntent.putExtra("userScreenName", mDirectMessage.getOtherUserScreenName());
			}
		} 
