...
public class AppSettings {
	...
	public void refresh(String preferenceKey) {
		...
		String theme = mSharedPreferences.getString(SettingsActivity.KEY_THEME_PREFERENCE, THEME_DEFAULT);
		setCurrentTheme(theme.equals(THEME_LIGHT) ? Theme.Holo_Light : Theme.Holo_Dark);
		
		String statusSize = mSharedPreferences.getString(SettingsActivity.KEY_STATUS_SIZE_PREFERENCE, STATUS_SIZE_DEFAULT);
		setCurrentStatusSize(statusSize);
		
		String profileImageSize = mSharedPreferences.getString(SettingsActivity.KEY_PROFILE_IMAGE_SIZE_PREFERENCE, PROFILE_IMAGE_SIZE_DEFAULT);
		setCurrentProfileImageSize(profileImageSize);
		
		//String nameDisplay = mSharedPreferences.getString(SettingsActivity.KEY_NAME_DISPLAY_PREFERENCE, NAME_DISPLAY_DEFAULT);
		//setCurrentNameDisplay(nameDisplay);
		
		String quoteType = mSharedPreferences.getString(SettingsActivity.KEY_QUOTE_TYPE_PREFERENCE, QUOTE_TYPE_DEFAULT);
		setCurrentQuoteType(quoteType);
		
		if (mRefreshCount > 0) {
			if (oldTheme != mCurrentTheme || oldStatusSize != mStatusSize || oldProfileImageSize != mProfileImageSize) {
				mIsDirty = true;
			} else if (preferenceKey != null) {
				if (preferenceKey.equalsIgnoreCase(SettingsActivity.KEY_SHOW_TABLET_MARGIN_PREFERENCE)
					//|| preferenceKey.equalsIgnoreCase(SettingsActivity.KEY_NAME_DISPLAY_PREFERENCE)
					|| preferenceKey.equalsIgnoreCase(SettingsActivity.KEY_CLEAR_IMAGE_CACHE_PREFERENCE)
					|| preferenceKey.equalsIgnoreCase(SettingsActivity.KEY_CUSTOMIZE_LANES_PREFERENCE)
					|| preferenceKey.equalsIgnoreCase(SettingsActivity.KEY_SHOW_TWEET_SOURCE_PREFERENCE)) {
					mIsDirty = true;
				}
			}
	...
	public boolean downloadFeedImages() {
		return mSharedPreferences.getBoolean(SettingsActivity.KEY_DOWNLOADIMAGES_PREFERENCE, DEFAULT_DOWNLOAD_IMAGES);
	}
	...
	public boolean showTweetSource() {
		return mSharedPreferences.getBoolean(SettingsActivity.KEY_SHOW_TWEET_SOURCE_PREFERENCE, DEFAULT_SHOW_TWEET_SOURCE);
	}
	...
	public boolean isVolScrollEnabled() {
		return mSharedPreferences.getBoolean(SettingsActivity.KEY_VOLSCROLL_PREFERENCE, DEFAULT_VOLSCROLL);
	}
	...
	public boolean isDimScreenEnabled() {
		//return mSharedPreferences.getBoolean(SettingsActivity.KEY_DIMSCREEN_PREFERENCE, DEFAULT_DIMSCREEN);
		return true;
	}
	...
	public boolean showTabletMargin() {
		if (mSharedPreferences != null && mSharedPreferences.contains(SettingsActivity.KEY_SHOW_TABLET_MARGIN_PREFERENCE)) {
			return mSharedPreferences.getBoolean(SettingsActivity.KEY_SHOW_TABLET_MARGIN_PREFERENCE, DEFAULT_SHOW_TABLET_MARGIN);
		}
	...
	public int getCurrentThemeStyle() {
		return mCurrentTheme == Theme.Holo_Dark ? R.style.Theme_TweetLanes : R.style.Theme_TweetLanes_Light;
	}
