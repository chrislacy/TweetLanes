...
public class EditClearText extends EditText{
	...
	private Drawable mClearButtonImage = getResources().getDrawable(AppSettings.get().getCurrentTheme() == AppSettings.Theme.Holo_Dark ? R.drawable.ic_navigation_cancel_dark : R.drawable.ic_navigation_cancel_light);
	...
	void init() {
		...
		// Manually set the padding. For some reason this doesn't seem to want to work when set in the XML file
		...
		setOnTouchListener(new OnTouchListener() {
			...
			public boolean onTouch(View v, MotionEvent event) {
				...
				if (canClearText == true
						&& event.getX() > getWidth() - getPaddingRight() - mClearButtonImage.getIntrinsicWidth()
						&& event.getY() > getHeight() - getPaddingBottom() - mClearButtonImage.getIntrinsicHeight()) {
	...
	void handleClearButton() {
		if (getText().toString().equals("")) {
			...
			//remove clear button
			setCompoundDrawables(getCompoundDrawables()[0], getCompoundDrawables()[1], mClearButtonImage, getCompoundDrawables()[3]);
		}
	}
	...
	protected void onSizeChanged(int xNew, int yNew, int xOld, int yOld) {
		...
		if (mInitialXSize == null && mInitialYSize == null) {
			mInitialXSize = xNew;
			mInitialYSize = Math.min(yNew, 75);	// TODO: Magic number. Find a better solution when the box is initialised with multiple lines 
			firstTime = true;
		}
		
		// Resize the clear button so it doesn't grow larger than the EditText field itself
		...
		mClearButtonImage.setBounds(0, buttonYOffset, buttonSize, buttonYOffset + buttonSize);
		...
		if (mListener != null) {
			mListener.onSizeChanged(xNew, yNew, xOld, yOld, mInitialXSize, mInitialYSize);
		}
	}
	...
	public interface EditClearTextListener {
		...
		/*
		 * Triggered when the view is touched. Use as a replacement for setOnTouchListener(), which EditClearText overrides
		 */
