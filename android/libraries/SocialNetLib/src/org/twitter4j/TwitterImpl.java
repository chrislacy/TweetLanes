...
import static org.twitter4j.internal.http.HttpParameter.getParameterArray;
...
class TwitterImpl extends TwitterBaseImpl implements Twitter {
    ...
    public ResponseList<Status> getMentions() throws TwitterException {
        ensureAuthorizationEnabled();
        return factory.createStatusList(get(conf.getRestBaseURL() +
                "statuses/mentions_timeline.json?include_entities="
                + conf.isIncludeEntitiesEnabled() + "&include_rts=" + conf.isIncludeRTsEnabled()));
    }
     ...
     * {@inheritDoc}
     */
    public ResponseList<Status> getUserListStatuses(String ownerScreenName, String listSlug, Paging paging) throws TwitterException {
        return factory.createStatusList(get(conf.getRestBaseURL() + "lists/statuses.json", mergeParameters(paging.asPostParameterArray(Paging.SMCP, Paging.PER_PAGE)
                , new HttpParameter[]{new HttpParameter("slug", listSlug),new HttpParameter("owner_screen_name", ownerScreenName),
                INCLUDE_ENTITIES,
                INCLUDE_RTS})));
    }
    ...
    public ResponseList<Status> getFavorites(String id) throws TwitterException {
        ...
        return factory.createStatusList(get(conf.getRestBaseURL()
                + "favorites/list.json?include_entities="
                + conf.isIncludeEntitiesEnabled() + "&screen_name=" + id));
    }
    ...
    public ResponseList<Status> getFavorites(String id, Paging paging) throws TwitterException {
        ...
        return factory.createStatusList(get(conf.getRestBaseURL() + "favorites/list.json",
                mergeParameters(paging.asPostParameterArray()
                        , new HttpParameter[]{INCLUDE_ENTITIES, new HttpParameter("screen_name", id)})));
    }
