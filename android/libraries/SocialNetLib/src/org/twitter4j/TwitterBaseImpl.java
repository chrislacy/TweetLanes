...
import org.twitter4j.auth.Authorization;
import org.twitter4j.auth.AuthorizationFactory;
import org.twitter4j.auth.BasicAuthorization;
import org.twitter4j.auth.NullAuthorization;
import org.twitter4j.auth.OAuthAuthorization;
import org.twitter4j.auth.OAuthSupport;
import org.twitter4j.auth.RequestToken;
import org.twitter4j.conf.Configuration;
import org.twitter4j.internal.http.HttpClientWrapper;
import org.twitter4j.internal.http.HttpResponse;
import org.twitter4j.internal.http.HttpResponseEvent;
import org.twitter4j.internal.http.HttpResponseListener;
import org.twitter4j.internal.http.XAuthAuthorization;
import org.twitter4j.internal.json.z_T4JInternalFactory;
import org.twitter4j.internal.json.z_T4JInternalJSONImplFactory;
...
import static org.twitter4j.internal.http.HttpResponseCode.ENHANCE_YOUR_CLAIM;
import static org.twitter4j.internal.http.HttpResponseCode.SERVICE_UNAVAILABLE;
...
abstract class TwitterBaseImpl implements java.io.Serializable, OAuthSupport, HttpResponseListener {
    ...
    protected final void ensureAuthorizationEnabled() {
        if (!auth.isEnabled()) {
            throw new IllegalStateException(
                    "Authentication credentials are missing. See http://org.twitter4j.org/configuration.html for the detail.");
        }
    }
    ...
    protected final void ensureOAuthEnabled() {
        if (!(auth instanceof OAuthAuthorization)) {
            throw new IllegalStateException(
                    "OAuth required. Authentication credentials are missing. See http://org.twitter4j.org/configuration.html for the detail.");
        }
    }
