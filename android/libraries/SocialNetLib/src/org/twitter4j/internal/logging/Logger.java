...
package org.twitter4j.internal.logging;
...
public abstract class Logger {
    ...
    private static final String LOGGER_FACTORY_IMPLEMENTATION = "org.twitter4j.loggerFactory";

    static {
        LoggerFactory loggerFactory = null;
        //-Dorg.twitter4j.debug=true -Dorg.twitter4j.loggerFactory=org.twitter4j.internal.logging.StdOutLoggerFactory
        ...
        if (null == loggerFactory) {
            loggerFactory = getLoggerFactoryIfAvailable("org.slf4j.impl.StaticLoggerBinder", "org.twitter4j.internal.logging.SLF4JLoggerFactory");
        }
        // otherwise, use commons-logging if it's found in the classpath
        if (null == loggerFactory) {
            loggerFactory = getLoggerFactoryIfAvailable("org.apache.commons.logging.Log", "org.twitter4j.internal.logging.CommonsLoggingLoggerFactory");
        }
        // otherwise, use log4j if it's found in the classpath
        if (null == loggerFactory) {
            loggerFactory = getLoggerFactoryIfAvailable("org.apache.log4j.Logger", "org.twitter4j.internal.logging.Log4JLoggerFactory");
        }
        // on Google App Engine, use java.util.logging
        if (null == loggerFactory) {
            loggerFactory = getLoggerFactoryIfAvailable("com.google.appengine.api.urlfetch.URLFetchService", "org.twitter4j.internal.logging.JULLoggerFactory");
        }
