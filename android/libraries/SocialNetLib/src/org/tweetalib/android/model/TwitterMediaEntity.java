...
public class TwitterMediaEntity {
	...
	public enum Source {
		...
		INSTAGRAM,
		TWITPIC,
		LOCKERZ,			// Note that Plixi and Lockerz use the same API, so Plixi is treated as Lockerz
		YFROG,
		IMGUR,
		YOUTUBE,
	}
	...
	public static TwitterMediaEntity createMediaEntity(AdnPost post) {
		...
		// TODO: This is a hack, but for now just replace newlines with a space so the split works.
		String status = post.mText.replace("\n", " ").replace("<", "&lt;").replace(">", "&gt;");
		String[] tokens = status.split(" ");
        for (String token : tokens) {
            if (token.length() > 1) {
            	if (token.contains("http://") || token.contains("https://")) {
					TwitterMediaEntity entity = getTwitterMediaEntityFromUrl(token, token);
					if (entity != null) {
						return entity;
					}
            	}
	...
	private static TwitterMediaEntity getTwitterMediaEntityFromUrl(String tinyUrl, String expandedUrl) {
		String mediaUrl = getInstagramMediaUrl(expandedUrl);
		if (mediaUrl != null) {
			return new TwitterMediaEntity(Source.INSTAGRAM, mediaUrl, tinyUrl, expandedUrl);
		}
		
		mediaUrl = getTwitpicMediaUrl(expandedUrl);
		if (mediaUrl != null) {
			return new TwitterMediaEntity(Source.TWITPIC, mediaUrl, tinyUrl, expandedUrl);
		}	

		mediaUrl = getLockerzMediaUrl(expandedUrl);
		if (mediaUrl != null) {
			return new TwitterMediaEntity(Source.LOCKERZ, mediaUrl, tinyUrl, expandedUrl);
		}
		
		mediaUrl = getYfrogMediaUrl(expandedUrl);
		if (mediaUrl != null) {
			return new TwitterMediaEntity(Source.YFROG, mediaUrl, tinyUrl, expandedUrl);
		}
		
		mediaUrl = getImgurUrl(expandedUrl);
		if (mediaUrl != null) {
			return new TwitterMediaEntity(Source.IMGUR, mediaUrl, tinyUrl, expandedUrl);
		}
		
		mediaUrl = getYouTubeUrl(expandedUrl);
		if (mediaUrl != null) {
			return new TwitterMediaEntity(Source.YOUTUBE, mediaUrl, tinyUrl, expandedUrl);
		}
	...
	private static String getImgurUrl(String url) {
		...
		if (startIndex > -1) {
			...
			return code.replace(".png", "").replace(".gif", "").replace(".jpg", "");
		}
	 ...
	 * 
	 */
	private TwitterMediaEntity(Source source, String mediaCode, String url, String expandedUrl) {
		mSource = source;
	...
	private TwitterMediaEntity(MediaEntity mediaEntity) {
		...
		/*
		mSizeThumb = new SizeInfo(mediaEntity.getSizes().get(MediaEntity.Size.THUMB));
		mSizeSmall = new SizeInfo(mediaEntity.getSizes().get(MediaEntity.Size.SMALL));
		mSizeMedium = new SizeInfo(mediaEntity.getSizes().get(MediaEntity.Size.MEDIUM));
		mSizeLarge = new SizeInfo(mediaEntity.getSizes().get(MediaEntity.Size.LARGE));
		*/
	}
	...
	public String 	getExpandedUrl()	{ return mExpandedUrl; }
	...
	public String getMediaUrl(Size size) {
		...
		switch (mSource) {
			case TWITTER:
				switch (size) {
					case THUMB: return mMediaCode + ":thumb";
					case SMALL: return mMediaCode + ":small";
					case MEDIUM: return mMediaCode + ":medium";
					case LARGE: return mMediaCode + ":large";
				}
				break;
		
			case INSTAGRAM:
				switch (size) {
					case THUMB: return mMediaCode + "?size=t";
					case SMALL: return mMediaCode + "?size=t";
					case MEDIUM: return mMediaCode + "?size=m";
					case LARGE: return mMediaCode + "?size=l";
				}
				break;
				
			case TWITPIC:
				switch (size) {
					case THUMB: return "http://twitpic.com/show/mini/" + mMediaCode;
					case SMALL: return "http://twitpic.com/show/thumb/" + mMediaCode;
					case MEDIUM: return "http://twitpic.com/show/thumb/" + mMediaCode;
					case LARGE: return "http://twitpic.com/show/full/" + mMediaCode;
				}
				break;
				
			case LOCKERZ:
				switch (size) {
					case THUMB: return mMediaCode + "&size=thumbnail";
					case SMALL: return mMediaCode + "&size=small";
					case MEDIUM: return mMediaCode + "&size=medium";
					case LARGE: return mMediaCode + "&size=big";
				}
				
			case YFROG:
				switch (size) {
					// http://yfrog.com/page/api#a5
					case THUMB: return mMediaCode + ":small";
					case SMALL: return mMediaCode + ":small";
					case MEDIUM: return mMediaCode + ":iphone";
					case LARGE: return mMediaCode + ":medium";
				}
				
			case IMGUR:
				switch (size) {
					// http://webapps.stackexchange.com/a/16104
					case THUMB: return "http://i.imgur.com/" + mMediaCode + "s.png";
					case SMALL: return "http://i.imgur.com/" + mMediaCode + "b.png";
					case MEDIUM: return "http://i.imgur.com/" + mMediaCode + "m.png";
					case LARGE: return "http://i.imgur.com/" + mMediaCode + ".png";
				}
			
			case YOUTUBE: {
				switch (size) {
					case THUMB:
					case SMALL:	return "http://img.youtube.com/vi/" + mMediaCode + "/default.jpg";
					case MEDIUM: return "http://img.youtube.com/vi/" + mMediaCode + "/mqdefault.jpg";
					case LARGE: return "http://img.youtube.com/vi/" + mMediaCode + "/hqdefault.jpg";
				}
			}
		}
	...
	public class SizeInfo {
		...
		public SizeInfo(MediaEntity.Size size) {
	...
	public SizeInfo getSize(Size size) {
		switch (size) {
			case THUMB: return mSizeThumb;
			case SMALL: return mSizeSmall;
			case MEDIUM: return mSizeMedium;
			case LARGE: return mSizeLarge;
		}
		
		return mSizeThumb;
	}

	private SizeInfo mSizeThumb;
	private SizeInfo mSizeSmall;
	private SizeInfo mSizeMedium;
	private SizeInfo mSizeLarge;
	*/
}
