...
public class TwitterStatus implements Comparable<TwitterStatus> {
	...
	if (mIsRetweet == true) {
		...
		mUserMentions = TwitterUtil.getUserMentions(status.getRetweetedStatus()
	...
	if (compareString(mAuthorName, other.mAuthorName) == false) return false;
	if (compareString(mAuthorScreenName, other.mAuthorScreenName) == false) return false;
	...
	if (compareLong(mInReplyToStatusId, other.mInReplyToStatusId) == false) return false;
	if (compareLong(mInReplyToUserId, other.mInReplyToUserId) == false) return false;
	...
	// false)
	// return false;
	if (compareString(mStatusFullMarkup, other.mStatusFullMarkup) == false) return false;
	if (compareString(mSource, other.mSource) == false) return false;
	if (mUserId != other.mUserId) return false;
	if (compareString(mUserScreenName, other.mUserScreenName) == false) return false;
