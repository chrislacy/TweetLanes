...
public class TwitterStatuses {
	 ...
	 */
	/*
	public TwitterStatus getStatus(Iterator iterator) {
		if (iterator.finished() == false) {
			return get(iterator.mLastIndex);
		}
		return null;
	}*/
	...
	public void add(AdnPosts posts, AddUserCallback addUserCallback) {
		...
		for (AdnPost post : posts.mPosts) {
		    ...
		    if (addUserCallback != null) {
		    	...
		    	/*
		    	if (post.isRetweet()) {
		    		Status retweetedStatus = post.getRetweetedStatus();
		    		if (retweetedStatus != null) {
		    			addUserCallback.addUser(retweetedStatus.getUser());
		    		}
		    	}*/
	...
	public class Iterator {
		...
		public Iterator(TwitterStatusesFilter filter) {
			mFilter = filter;
			mLastIndex = 0;
		}
		...
		public void next() {
			int size = size();
			for (; mLastIndex < size; mLastIndex++) {
				TwitterStatus status = get(mLastIndex);
				if (status != null) {
					if (status.getInReplyToStatusId() != null && mFilter.getShowReplies() == false) {
						mSkipReplyCount += 1;
						continue;
					}
					if (status.getIsRetweet() == true && mFilter.getShowRetweets() == false) {
						mSkipRetweetCount += 1;
						continue;
					}
					break;
		...
		public boolean finished() {
			...
			if (mLastIndex == size()) {
				Log.d("TwitterStatus iterator", "finished - skipped " + mSkipRetweetCount + " retweets, skipped " + mSkipReplyCount + " replies.");
			}
			
			return mLastIndex == size() ? true : false;
		}
		
		private TwitterStatusesFilter mFilter;
		int mLastIndex;
		
		int mSkipRetweetCount = 0;
		int mSkipReplyCount = 0;
	}
	*/
	...
	private void filterTest(TwitterStatusesFilter filter) {
		...
		int count = getStatusCount(filter);
		for (int i = 0; i < count; i++) {
			TwitterStatus status = getStatus(i, filter);
			if (status == null) {
				getStatus(i, filter);
			}
		}
