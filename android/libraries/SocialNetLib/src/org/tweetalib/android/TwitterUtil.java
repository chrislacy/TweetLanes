...
public class TwitterUtil {
	...
	public static String stripMarkup(String text) {
		return text != null ? android.text.Html.fromHtml(text).toString() : null;
	}
	 ...
	 */
	public static String getStatusMarkup(Status status, TwitterMediaEntity twitterMediaEntity) {
        return getStatusMarkup(status.getText(), twitterMediaEntity, status.getMediaEntities(), status.getURLEntities());
	}
	 ...
	 * 
	 */
	public static String getStatusMarkup(AdnPost post, TwitterMediaEntity mediaEntity) {
	 ...
	 * 
	 */
	private static String getStatusMarkup(String statusText, TwitterMediaEntity twitterMediaEntity, MediaEntity[] mediaEntities, URLEntity[] urlEntities) {
		
		initCommon();
		
		String autoLinkAll = mAutolinkEx.autoLinkAll(statusText, twitterMediaEntity, mediaEntities, urlEntities);
		return autoLinkAll;
	}
	...
	/*
	public static String getStatusMarkup(String status, URLEntity[] urlEntities) {
		...
		if (urlEntities.length > 0) {
			...
			for (int i = 0; i < urlEntities.length; i++) {
				...
				// TODO: Problem with this tweet: https://twitter.com/#!/Proto3000_RP/status/177144322645434368
				// Have to keep an eye on this solution
				if (end > statusLength) {
				...
				if (start > 0) {
					if (result == null) {
						result = status.substring(0, start);
					} else if (lastEnd > -1) {
						if (lastEnd < start) {
							result += status.substring(lastEnd, start);
						} else {
							// TODO: Handle this, such as for id 179136028555231232
						}
					}
				}
				String link = "<a href=\"" + urlEntity.getExpandedURL() + "\">" + urlEntity.getDisplayURL() + "</a>";
				result += link;
				String after;
				if (i == urlEntities.length - 1) {
					if (end <= statusLength) {
						after = status.substring(end, statusLength);
					} else {
						after = "";		// should never get here
					}
				} else {
					after = status.substring(end, urlEntities[i+1].getStart());
				}
				result += after;
				lastEnd = end;
			}
	 ...
	 * 
	 */
	public static Date iso6801StringToDate(String iso8601string) throws ParseException {
