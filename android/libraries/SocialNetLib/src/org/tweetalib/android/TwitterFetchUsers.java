...
public class TwitterFetchUsers {
	...
	new FetchUsersTask().execute(AsyncTaskEx.PRIORITY_MEDIUM, "Fetch Users",
		new FetchUsersTaskInput(mFetchUsersCallbackHandle, contentHandle, connectionStatus,
			paging));
	 ...
	 * 
	 */
    public void updateFriendshipUser(String currentUserScreenName, TwitterUser userToUpdate,
	    boolean create, FinishedCallback callback, ConnectionStatus connectionStatus) {
	updateFriendshipUsers(currentUserScreenName, new TwitterUsers(userToUpdate), create,
		callback, connectionStatus);
    }

    public void updateFriendshipUsers(String currentUserScreenName, TwitterUsers usersToUpdate,
	    boolean create, FinishedCallback callback, ConnectionStatus connectionStatus) {
	...
	updateFriendshipScreenNames(currentUserScreenName, userScreenNames, create, callback,
		connectionStatus);
    }
    ...
    public void updateFriendshipScreenNames(String currentUserScreenName,
	    ArrayList<String> userScreenNamesToUpdate, boolean create, FinishedCallback callback,
	...
	TwitterContentHandle contentHandle = new TwitterContentHandle(new TwitterContentHandleBase(
		TwitterConstant.ContentType.USERS, TwitterConstant.UsersType.UPDATE_FRIENDSHIP),
	...
	new FetchUsersTask().execute(AsyncTaskEx.PRIORITY_MEDIUM, "Update Friendships",
		new FetchUsersTaskInput(mFetchUsersCallbackHandle, contentHandle, connectionStatus,
			null, userIdsToUpdate, create));
    }

    /*
	 * 
	 */
    private void createBlockOrReportSpam(UsersType usersType, long currentUserId, Long userId,
	    FinishedCallback callback, ConnectionStatus connectionStatus) {
	ArrayList<Long> userIds = new ArrayList<Long>();
	userIds.add(userId);
    ...
    private void createBlockOrReportSpam(UsersType usersType, long currentUserId,
	    ArrayList<Long> userIds, FinishedCallback callback, ConnectionStatus connectionStatus) {
	if (connectionStatus.isOnline() == false) {
	    if (callback != null) {
		callback.finished(
			new TwitterFetchResult(false, connectionStatus
	...
	_mFriendshipCounter += 1;
	TwitterContentHandle contentHandle = new TwitterContentHandle(new TwitterContentHandleBase(
		TwitterConstant.ContentType.USERS, usersType), Long.toString(currentUserId),
	 ...
	 * 
	 */
    public void reportSpam(long currentUserId, Long userId, FinishedCallback callback,
	    ConnectionStatus connectionStatus) {
	createBlockOrReportSpam(UsersType.REPORT_SPAM, currentUserId, userId, callback,
		connectionStatus);
    }

    public void reportSpam(long currentUserId, ArrayList<Long> userIds, FinishedCallback callback,
	    ConnectionStatus connectionStatus) {
	createBlockOrReportSpam(UsersType.REPORT_SPAM, currentUserId, userIds, callback,
		connectionStatus);
    }
	 ...
	 * 
	 */
    public void createBlock(long currentUserId, Long userId, FinishedCallback callback,
	    ConnectionStatus connectionStatus) {
	createBlockOrReportSpam(UsersType.CREATE_BLOCK, currentUserId, userId, callback,
		connectionStatus);
    }

    public void createBlock(long currentUserId, ArrayList<Long> userIds, FinishedCallback callback,
	    ConnectionStatus connectionStatus) {
	createBlockOrReportSpam(UsersType.CREATE_BLOCK, currentUserId, userIds, callback,
		connectionStatus);
    }
	...
	FetchUsersTaskInput(Integer callbackHandle, TwitterContentHandle contentHandle,
	...
	FetchUsersTaskInput(Integer callbackHandle, TwitterContentHandle contentHandle,
		ConnectionStatus connectionStatus, ArrayList<String> userScreenNames,
		ArrayList<Long> userIds, boolean createFriendship) {
	    mCallbackHandle = callbackHandle;
	    mContentHandle = contentHandle;
	    mConnectionStatus = connectionStatus;
	...
	protected FetchUsersTaskOutput doInBackground(FetchUsersTaskInput... inputArray) {
		...
		try {
			...
			if (input.mScreenNames != null) {
			    ...
			    long currentUserId = Long.parseLong(input.mContentHandle
				    .getScreenName());
	    ...
	    return new FetchUsersTaskOutput(new TwitterFetchResult(errorDescription == null ? true
		    : false, errorDescription), input.mCallbackHandle, twitterUsers);
	}
