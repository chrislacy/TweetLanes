...
public class TwitterSignIn {
	 ...
	 * 
	 */
	public abstract class GetAuthUrlCallback implements GetAuthUrlCallbackInterface {
	 ...
	 * 
	 */
	public abstract class GetOAuthAccessTokenCallback implements GetOAuthAccessTokenCallbackInterface {
	...
	GetAuthUrlCallback getAuthUrlCallback(Integer callbackHandle) {
		GetAuthUrlCallback callback = mGetAuthUrlCallbackMap.get(callbackHandle);
		return callback;
	}
	 ...
	 * 
	 */
	GetOAuthAccessTokenCallback getOAuthAccessTokenCallback(Integer callbackHandle) {
		GetOAuthAccessTokenCallback callback = mGetOAuthAccessTokenCallbackMap.get(callbackHandle);
		return callback;
	}
	...
	public void getAuthUrl(GetAuthUrlCallback callback) {
		...
		new FetchAuthUrlTask().execute(AsyncTaskEx.PRIORITY_HIGHEST, "Get Auth URL",
				new FetchAuthUrlTaskInput(mGetAuthUrlCallbackHandle));
	 ...
	 * 
	 */
	public void getOAuthAccessToken(RequestToken requestToken, String oauthVerifier, GetOAuthAccessTokenCallback callback) {
		
		assert(mGetOAuthAccessTokenCallbackMap.containsValue(callback) == false);
		
		mGetOAuthAccessTokenCallbackMap.put(mGetOAuthAccessTokenCallbackHandle, callback);
		new FetchOAuthAccessTokenTask().execute(AsyncTaskEx.PRIORITY_HIGHEST, "Get OAuth AccessToken",
				new FetchOAuthAccessTokenTaskInput(requestToken, oauthVerifier, mGetOAuthAccessTokenCallbackHandle));
	...
	class FetchAuthUrlTaskOutput {
		...
		FetchAuthUrlTaskOutput(Integer callbackHandle, RequestToken requestToken, String url) {
			mCallbackHandle = callbackHandle;
	 ...
	 * 
	 */
	class FetchAuthUrlTask extends AsyncTaskEx<FetchAuthUrlTaskInput, Void, FetchAuthUrlTaskOutput> {

		@Override
		protected FetchAuthUrlTaskOutput doInBackground(FetchAuthUrlTaskInput... inputArray) {
			...
			twitter.setOAuthConsumer(mCallbacks.getConsumerKey(), mCallbacks.getConsumerSecret());
			...
			return new FetchAuthUrlTaskOutput(input.mCallbackHandle, requestToken, url);
		}
	...
	class FetchOAuthAccessTokenTaskInput {
		...
		FetchOAuthAccessTokenTaskInput(RequestToken requestToken, String oauthVerifier, Integer callbackHandle) {
	...
	class FetchOAuthAccessTokenTaskOutput {
		...
		FetchOAuthAccessTokenTaskOutput(TwitterUser user, String accessToken, String accessTokenSecret, Integer callbackHandle) {
	 ...
	 * 
	 */
	class FetchOAuthAccessTokenTask extends AsyncTaskEx<FetchOAuthAccessTokenTaskInput, Void, FetchOAuthAccessTokenTaskOutput> {

		@Override
		protected FetchOAuthAccessTokenTaskOutput doInBackground(FetchOAuthAccessTokenTaskInput... inputArray) {
			...
			try
			{
		        ...
		        switch (mCallbacks.getType()) {
			        case Twitter:
						...
						twitter.setOAuthConsumer(mCallbacks.getConsumerKey(), mCallbacks.getConsumerSecret());
						AccessToken at = twitter.getOAuthAccessToken(input.mRequestToken, input.mOAuthVerifier);
		        ...
		        TwitterUser user = mCallbacks.verifyCredentials(accessToken, accessTokenSecret);
		        
		        return new FetchOAuthAccessTokenTaskOutput(user, accessToken, accessTokenSecret, input.mCallbackHandle);
			}
			catch (TwitterException e)
			{
			     e.printStackTrace();
			...
			return new FetchOAuthAccessTokenTaskOutput(null, null, null, input.mCallbackHandle);
		}
		...
		protected void onPostExecute(FetchOAuthAccessTokenTaskOutput output) {
			...
			if (callback != null) {
				if (output.mUser != null && output.mAccessToken != null) {
					callback.finished(true, output.mUser, output.mAccessToken, output.mAccessTokenSecret);
				} else {
