...
public class TwitterFetchDirectMessages {
	 ...
	 * 
	 */
	TwitterFetchDirectMessagesFinishedCallback getFetchStatusesCallback(Integer callbackHandle) {
		TwitterFetchDirectMessagesFinishedCallback callback = mFinishedCallbackMap.get(callbackHandle);
		return callback;
	}
	 ...
	 * 
	 */
	void removeFetchStatusesCallback(TwitterFetchDirectMessagesFinishedCallback callback) {
	...
	public TwitterDirectMessages getDirectMessages(TwitterContentHandle contentHandle) {
		...
		if (mMessagesHashMap != null) {
			TwitterDirectMessages messages = mMessagesHashMap.get(contentHandle.getKey());
			if (messages == null) {
	 ...
	 * 
	 */
	public TwitterDirectMessages getDirectMessages(TwitterContentHandle contentHandle, TwitterPaging paging, TwitterFetchDirectMessagesFinishedCallback callback, ConnectionStatus connectionStatus) {
		...
		new FetchStatusesTask().execute(AsyncTaskEx.PRIORITY_MEDIUM, "Fetch DMs",
				new FetchDirectMessagesTaskInput(mFetchMessagesCallbackHandle, contentHandle, paging, connectionStatus));
	...
	public void sendDirectMessage(long userId, String recipientScreenName, String statusText, 
			TwitterFetchDirectMessagesFinishedCallback callback, ConnectionStatus connectionStatus) {
		if (connectionStatus.isOnline() == false) {
			if (callback != null) {
				callback.finished(new TwitterFetchResult(false, connectionStatus.getErrorMessageNoConnection()), null);
			}
		...
		mFinishedCallbackMap.put(mFetchMessagesCallbackHandle, callback);
		new FetchStatusesTask().execute(AsyncTaskEx.PRIORITY_MEDIUM, "Fetch DMs",
				new FetchDirectMessagesTaskInput(mFetchMessagesCallbackHandle, userId, recipientScreenName, statusText, connectionStatus));
		mFetchMessagesCallbackHandle += 1;
	}	
	...
	class FetchDirectMessagesTaskInput {
		...
		FetchDirectMessagesTaskInput(Integer callbackHandle, TwitterContentHandle contentHandle, TwitterPaging paging, ConnectionStatus connectionStatus) {
		...
		FetchDirectMessagesTaskInput(Integer callbackHandle, Long userId, String recipientScreenName, String statusText, 
	...
	class FetchDirectMessagesTaskOutput {
		...
		FetchDirectMessagesTaskOutput(TwitterFetchResult result, Integer callbackHandle, TwitterDirectMessages messages) {
			mResult = result;
			mCallbackHandle = callbackHandle;
			mMessages = messages;
		}
	 ...
	 * 
	 */
	class FetchStatusesTask extends AsyncTaskEx<FetchDirectMessagesTaskInput, Void, FetchDirectMessagesTaskOutput> {

		@Override
		protected FetchDirectMessagesTaskOutput doInBackground(FetchDirectMessagesTaskInput... inputArray) {
			...
			if (input.mConnectionStatus.isOnline() == false) {
				return new FetchDirectMessagesTaskOutput(new TwitterFetchResult(false, input.mConnectionStatus.getErrorMessageNoConnection()), input.mCallbackHandle, null);
			}
			...
			if (twitter != null) {
				...
				try {
					if (input.mStatusText != null) {
						...
						DirectMessage dm = twitter.sendDirectMessage(input.mRecipientScreenName, input.mStatusText);
						...
						switch (input.mContentHandle.getDirectMessagesType()) {
							case ALL_MESSAGES:
							{
								...
								// Annoyingly, DMs can't be retrieved in a threaded format. Handle this by getting sent and received and managing ourselves...
								ResponseList<DirectMessage> receivedDirectMessages = twitter.getDirectMessages(paging);
								ResponseList<DirectMessage> sentDirectMessages = twitter.getSentDirectMessages(paging);
								...
								messages.add(sentDirectMessages, receivedDirectMessages, addUserCallback);
								break;
							}
							default:
								break;
			...
			return new FetchDirectMessagesTaskOutput(new TwitterFetchResult(errorDescription == null ? true : false, errorDescription), input.mCallbackHandle, messages);
		}
