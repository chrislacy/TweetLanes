...
public class TwitterFetchBooleans {
	 ...
	 * 
	 */
	public void getFriendshipExists(String userScreenName, String userScreenNameToCheck, FinishedCallback callback, ConnectionStatus connectionStatus) {

		triggerFetchBooleanTask(new FetchBooleanTaskInput(mFetchBooleanCallbackHandle, connectionStatus, BooleanType.FRIENDSHIP_EXISTS, userScreenName, userScreenNameToCheck), callback, connectionStatus);
	}
	
	/*
	 * 
	 */
	void triggerFetchBooleanTask(FetchBooleanTaskInput taskInput, FinishedCallback callback, ConnectionStatus connectionStatus) {
		...
		new FetchBooleanTask().execute(AsyncTaskEx.PRIORITY_MEDIUM, "Fetch Bools", taskInput);
	...
	class FetchBooleanTaskInput {
		...
		FetchBooleanTaskInput(Integer callbackHandle, ConnectionStatus connectionStatus, BooleanType booleanType, String userScreenName, String userScreenNameToCheck) {
	...
	class FetchBooleanTaskOutput {
		...
		FetchBooleanTaskOutput(TwitterFetchResult result, Integer callbackHandle, ArrayList<Boolean> returnValues) {
			mResult = result;
			mCallbackHandle = callbackHandle;
	 ...
	 * 
	 */
	class FetchBooleanTask extends AsyncTaskEx<FetchBooleanTaskInput, Void, FetchBooleanTaskOutput> {

		@Override
		protected FetchBooleanTaskOutput doInBackground(FetchBooleanTaskInput... inputArray) {
			...
			if (input.mConnectionStatus.isOnline() == false) {
				return new FetchBooleanTaskOutput(new TwitterFetchResult(false, input.mConnectionStatus.getErrorMessageNoConnection()), input.mCallbackHandle, null);
			}
			...
			return new FetchBooleanTaskOutput(new TwitterFetchResult(errorDescription == null ? true : false, errorDescription), input.mCallbackHandle, result);
		}
