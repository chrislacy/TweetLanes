 ...
 * intent that always includes data about the calling activity and app. This
 * lets the called activity provide attribution for the app that shared content.
 * Constructing an intent this way can be done in a method-chaining style. To
 * obtain an IntentBuilder with info about your calling activity, use the static
 * method {@link IntentBuilder#from(Activity)}.
 * 
 * {@link IntentReader} provides helper functions for parsing the defined extras
 * within an {@link Intent#ACTION_SEND} or {@link Intent#ACTION_SEND_MULTIPLE}
 * intent used to launch an activity. You can also obtain a Drawable for the
 * caller's application icon and the application's localized label (the app's
 * human-readable name). Social apps that enable sharing content are encouraged
 * to use this information to call out the app that the content was shared from.
 */
public class ShareCompat {
    /**
     * Intent extra that stores the name of the calling package for an
     * ACTION_SEND intent. When an activity is started using
     * startActivityForResult this is redundant info. (It is also provided by
     * {@link Activity#getCallingPackage()}.)
     * 
     * Instead of using this constant directly, consider using
     * {@link #getCallingPackage(Activity)} or
     * {@link IntentReader#getCallingPackage()}.
     */
    public static final String EXTRA_CALLING_PACKAGE = "android.support.v4.app.EXTRA_CALLING_PACKAGE";

    /**
     * Intent extra that stores the {@link ComponentName} of the calling
     * activity for an ACTION_SEND intent.
     */
    ...
    /**
     * Retrieve the name of the package that launched calledActivity from a
     * share intent. Apps that provide social sharing functionality can use this
     * to provide attribution for the app that shared the content.
     * 
     * <p>
     * <em>Note:</em> This data may have been provided voluntarily by the
     * calling application. As such it should not be trusted for accuracy in the
     * context of security or verification.
     * </p>
     * 
    ...
    public static String getCallingPackage(Activity calledActivity) {
        ...
        if (result == null) {
            result = calledActivity.getIntent().getStringExtra(
    ...
    /**
     * Retrieve the ComponentName of the activity that launched calledActivity
     * from a share intent. Apps that provide social sharing functionality can
     * use this to provide attribution for the app that shared the content.
     * 
     * <p>
     * <em>Note:</em> This data may have been provided voluntarily by the
     * calling application. As such it should not be trusted for accuracy in the
     ...
     * 
     * <p>
     * If the app is running on API level 14 or higher (Android 4.0/Ice Cream
     * Sandwich) this method will configure a ShareActionProvider to provide a
     * more robust UI for selecting the target of the share. History will be
     * tracked for each calling activity in a file named with the prefix
     * ".sharecompat_" in the application's private data directory. If the
     * application wishes to set this MenuItem to show as an action in the
     * Action Bar it should use
     * {@link MenuItemCompat#setShowAsAction(MenuItem, int)} to request that
     * behavior in addition to calling this method.
     * </p>
     * 
     * <p>
     * If the app is running on an older platform version this method will
     * configure a standard activity chooser dialog for the menu item.
     * </p>
     * 
     * <p>
     * During the calling activity's lifecycle, if data within the share intent
     * must change the app should change that state in one of several ways:
     * </p>
     * <ul>
     * <li>Call {@link ActivityCompat#invalidateOptionsMenu(Activity)}. If the
     * app is running on API level 11 or above and uses the Action Bar its menu
     * will be recreated and rebuilt. If not, the activity will receive a call
     * to {@link Activity#onPrepareOptionsMenu(Menu)} the next time the user
     * presses the menu key to open the options menu panel. The activity can
     * then call configureMenuItem again with a new or altered IntentBuilder to
     * reconfigure the share menu item.</li>
     * <li>Keep a reference to the MenuItem object for the share item once it
     * has been created and call configureMenuItem to update the associated
     * sharing intent as needed.</li>
     * </ul>
     * 
     * @param item
     ...
     */
    public static void configureMenuItem(Menu menu, int menuItemId,
            IntentBuilder shareIntent) {
        MenuItem item = menu.findItem(menuItemId);
        if (item == null) {
            throw new IllegalArgumentException(
                    "Could not find menu item with id " + menuItemId
     ...
     * {@link Intent#ACTION_SEND_MULTIPLE} sharing intents and starting
     * activities to share content. The ComponentName and package name of the
     * calling activity will be included.
     */
    public static class IntentBuilder {
        ...
        /**
         * Create a new IntentBuilder for launching a sharing action from
         * launchingActivity.
         * 
        ...
        private IntentBuilder(Activity launchingActivity) {
            ...
            mIntent.putExtra(EXTRA_CALLING_PACKAGE,
                    launchingActivity.getPackageName());
            mIntent.putExtra(EXTRA_CALLING_ACTIVITY,
                    launchingActivity.getComponentName());
            mIntent.addFlags(Intent.FLAG_ACTIVITY_CLEAR_WHEN_TASK_RESET);
        }

        /**
         * Retrieve the Intent as configured so far by the IntentBuilder. This
         * Intent is suitable for use in a ShareActionProvider or chooser
         * dialog.
         * 
         * <p>
         * To create an intent that will launch the activity chooser so that the
         * user may select a target for the share, see
         * {@link #createChooserIntent()}.
         * 
        ...
        public Intent getIntent() {
            ...
            boolean isSendMultiple = mIntent.getAction().equals(
                    Intent.ACTION_SEND_MULTIPLE);

            if (!needsSendMultiple && isSendMultiple) {
                // Change back to a single send action; place the first stream
                // into the
            ...
            if (needsSendMultiple && !isSendMultiple) {
                // Change to a multiple send action; place the relevant
                // ArrayList into the
        ...
        private void combineArrayExtra(String extra, ArrayList<String> add) {
            ...
            int currentLength = currentAddresses != null ? currentAddresses.length
                    : 0;
            String[] finalAddresses = new String[currentLength + add.size()];
            add.toArray(finalAddresses);
            if (currentAddresses != null) {
                System.arraycopy(currentAddresses, 0, finalAddresses,
                        add.size(), currentLength);
            }
        ...
        /**
         * Create an Intent that will launch the standard Android activity
         * chooser, allowing the user to pick what activity/app on the system
         * should handle the share.
         * 
         ...
         * ShareCompat.configureMenuItem()} to add a Share item to the menu
         * while presenting a detail view of the content to be shared instead of
        ...
        /**
         * Set the title that will be used for the activity chooser for this
         * share.
         * 
         * @param title
         *            Title string
         ...
         * 
         * <p>
         * This replaces all currently set stream URIs and will produce a
         * single-stream ACTION_SEND intent.
         * </p>
         * 
         * @param streamUri
        ...
        /**
         * Add a stream URI to the data that should be shared. If this is not
         * the first stream URI added the final intent constructed will become
         * an ACTION_SEND_MULTIPLE intent. Not all apps will handle both
        ...
        /**
         * Set an array of email addresses as recipients of this share. This
         * replaces all current "to" recipients that have been set so far.
         * 
         * @param addresses
        ...
        /**
         * Add an email address to be used in the "to" field of the final
         * Intent.
         * 
         * @param address
        ...
        /**
         * Add an array of email addresses to be used in the "to" field of the
         * final Intent.
         * 
         * @param addresses
         *            Email addresses to send to
        ...
        /**
         * Set an array of email addresses to CC on this share. This replaces
         * all current "CC" recipients that have been set so far.
         * 
         * @param addresses
        ...
        /**
         * Add an email address to be used in the "cc" field of the final
         * Intent.
         * 
         * @param address
         *            Email address to CC
        ...
        /**
         * Add an array of email addresses to be used in the "cc" field of the
         * final Intent.
         * 
         * @param addresses
        ...
        /**
         * Set an array of email addresses to BCC on this share. This replaces
         * all current "BCC" recipients that have been set so far.
         * 
         * @param addresses
        ...
        /**
         * Add an email address to be used in the "bcc" field of the final
         * Intent.
         * 
         * @param address
        ...
        /**
         * Add an array of email addresses to be used in the "bcc" field of the
         * final Intent.
         * 
         * @param addresses
    ...
    /**
     * IntentReader is a helper for reading the data contained within a sharing
     * (ACTION_SEND) Intent. It provides methods to parse standard elements
     * included with a share in addition to extra metadata about the app that
     * shared the content.
     * 
     * <p>
     * Social sharing apps are encouraged to provide attribution for the app
     * that shared the content. IntentReader offers access to the application
     * label, calling activity info, and application icon of the app that shared
     * the content. This data may have been provided voluntarily by the calling
     * app and should always be displayed to the user before submission for
     * manual verification. The user should be offered the option to omit this
     * information from shared posts if desired.
     * </p>
     * 
     * <p>
     * Activities that intend to receive sharing intents should configure an
     * intent-filter to accept {@link Intent#ACTION_SEND} intents
     * ("android.intent.action.SEND") and optionally accept
     * {@link Intent#ACTION_SEND_MULTIPLE}
     * ("android.intent.action.SEND_MULTIPLE") if the activity is equipped to
    ...
    public static class IntentReader {
        ...
        /**
         * Returns true if the activity this reader was obtained for was started
         * with an {@link Intent#ACTION_SEND} or
         * {@link Intent#ACTION_SEND_MULTIPLE} sharing Intent.
         * 
         * @return true if the activity was started with an ACTION_SEND or
         *         ACTION_SEND_MULTIPLE Intent
         */
        public boolean isShareIntent() {
            final String action = mIntent.getAction();
            return action.equals(Intent.ACTION_SEND)
                    || action.equals(Intent.ACTION_SEND_MULTIPLE);
        }

        /**
         * Returns true if the activity this reader was obtained for was started
         * with an {@link Intent#ACTION_SEND} intent and contains a single
         * shared item. The shared content should be obtained using either the
         * {@link #getText()} or {@link #getStream()} methods depending on the
        ...
        /**
         * Returns true if the activity this reader was obtained for was started
         * with an {@link Intent#ACTION_SEND_MULTIPLE} intent. The Intent may
         * contain more than one stream item.
         * 
         * @return true if the activity was started with an ACTION_SEND_MULTIPLE
         *         intent
         */
         ...
         * 
         * <p>
         * This call will fail if the share intent contains multiple stream
         * items. If {@link #isMultipleShare()} returns true the application
         * should use {@link #getStream(int)} and {@link #getStreamCount()} to
         * retrieve the included stream items.
         * </p>
         * 
         * @return A URI referring to a data stream to be shared or null if one
         *         was not supplied
         * @see Intent#EXTRA_STREAM
         */
        ...
        /**
         * Get the URI of a stream item shared with the target activity. Index
         * should be in the range [0-getStreamCount()).
         * 
         * @param index
        ...
        public Uri getStream(int index) {
            ...
            throw new IndexOutOfBoundsException("Stream items available: "
                    + getStreamCount() + " index requested: " + index);
        }

        /**
         * Return the number of stream items shared. The return value will be 0
         * or 1 if this was an {@link Intent#ACTION_SEND} intent, or 0 or more
         * if it was an {@link Intent#ACTION_SEND_MULTIPLE} intent.
         * 
         ...
         * 
         * @return The subject heading for this share or null if one was not
         *         supplied.
         * @see Intent#EXTRA_SUBJECT
         */
        ...
        /**
         * Get the name of the package that invoked this sharing intent. If the
         * activity was not started for a result, IntentBuilder will read this
         * from extra metadata placed in the Intent by ShareBuilder.
         * 
         * <p>
         * <em>Note:</em> This data may have been provided voluntarily by the
         * calling application. As such it should not be trusted for accuracy in
         * the context of security or verification.
         * </p>
         * 
         * @return Name of the package that started this activity or null if
         *         unknown
        ...
        /**
         * Get the {@link ComponentName} of the Activity that invoked this
         * sharing intent. If the target sharing activity was not started for a
         * result, IntentBuilder will read this from extra metadata placed in
         * the intent by ShareBuilder.
         * 
         * <p>
         * <em>Note:</em> This data may have been provided voluntarily by the
         * calling application. As such it should not be trusted for accuracy in
        ...
        /**
         * Get the icon of the calling activity as a Drawable if data about the
         * calling activity is available.
         * 
         * <p>
         * <em>Note:</em> This data may have been provided voluntarily by the
         * calling application. As such it should not be trusted for accuracy in
        ...
        /**
         * Get the icon of the calling application as a Drawable if data about
         * the calling package is available.
         * 
         * <p>
         * <em>Note:</em> This data may have been provided voluntarily by the
         * calling application. As such it should not be trusted for accuracy in
        ...
        public CharSequence getCallingApplicationLabel() {
            ...
            try {
                ...
                Log.e(TAG, "Could not retrieve label for calling application",
                        e);
            }
            return null;
        }
