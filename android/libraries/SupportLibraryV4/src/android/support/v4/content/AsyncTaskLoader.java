...
public abstract class AsyncTaskLoader<D> extends Loader<D> {
    ...
    final class LoadTask extends ModernAsyncTask<Void, Void, D> implements
            Runnable {
     ...
     * {@link android.os.AsyncTask#cancel(boolean)} for more info. Must be
     * called on the main thread of the process.
     * 
     * <p>
        ...
        if (mCancellingTask == null && mTask != null) {
            ...
            if (mUpdateThrottle > 0) {
                ...
                if (now < (mLastLoadCompleteTime + mUpdateThrottle)) {
                    ...
                    if (DEBUG)
                        Log.v(TAG, "Waiting until "
                                + (mLastLoadCompleteTime + mUpdateThrottle)
                                + " to execute: " + mTask);
                    mTask.waiting = true;
                    mHandler.postAtTime(mTask, mLastLoadCompleteTime
                            + mUpdateThrottle);
                    return;
                }
            }
            if (DEBUG) Log.v(TAG, "Executing: " + mTask);
            mTask.executeOnExecutor(ModernAsyncTask.THREAD_POOL_EXECUTOR,
                    (Void[]) null);
        }
    }
